...
#ifdef __EMSCRIPTEN__
static std::string TVPXP3TraceStorageNameUtf8(const ttstr &name)
{
	std::string out;
...
static bool TVPXP3TracePsbName(const ttstr &name)
{
	std::string n = TVPXP3TraceStorageNameUtf8(name);
	return n.find(".psb") != std::string::npos || n.find(".PSB") != std::string::npos;
}

static std::string TVPXP3LowerStorageName(const ttstr &name)
{
	std::string n = TVPXP3TraceStorageNameUtf8(name);
...
static bool TVPXP3EndsWith(const std::string &value, const char *suffix)
{
	size_t value_len = value.size();
	size_t suffix_len = strlen(suffix);
	return value_len >= suffix_len &&
		value.compare(value_len - suffix_len, suffix_len, suffix) == 0;
}

static bool TVPXP3IsAudioName(const std::string &n)
{
	return TVPXP3EndsWith(n, ".ogg") || TVPXP3EndsWith(n, ".opus") ||
		TVPXP3EndsWith(n, ".wav") || TVPXP3EndsWith(n, ".mp3") ||
		TVPXP3EndsWith(n, ".flac") || TVPXP3EndsWith(n, ".wma") ||
		TVPXP3EndsWith(n, ".aac") || TVPXP3EndsWith(n, ".m4a");
}

static bool TVPXP3IsVideoName(const std::string &n)
{
	return TVPXP3EndsWith(n, ".mpg") || TVPXP3EndsWith(n, ".mpeg") ||
		TVPXP3EndsWith(n, ".mp4") || TVPXP3EndsWith(n, ".webm") ||
		TVPXP3EndsWith(n, ".ogv") || TVPXP3EndsWith(n, ".avi") ||
		TVPXP3EndsWith(n, ".wmv") || TVPXP3EndsWith(n, ".m4v") ||
		TVPXP3EndsWith(n, ".flv") || TVPXP3EndsWith(n, ".mov") ||
		TVPXP3EndsWith(n, ".3gp");
}

static bool TVPXP3FlagDisabled(const char *name)
{
	tTJSVariant value;
...
static void TVPXP3ClassifyStorageName(const std::string &lowerItem,
	tTVPXP3ReadClass &readClass)
{
	bool isPsb = lowerItem.find(".psb") != std::string::npos;
	bool isAudio = TVPXP3IsAudioName(lowerItem);
	bool isVideo = TVPXP3IsVideoName(lowerItem);
	readClass = isPsb ? TVP_XP3_READ_PSB_FULL :
		(isVideo ? TVP_XP3_READ_VIDEO_STREAM :
		(isAudio ? TVP_XP3_READ_AUDIO_STREAM : TVP_XP3_READ_SMALL_RANDOM));
}

static const char * TVPXP3ModeName(tTVPXP3ReadClass cls, const char *suffix)
{
	bool segment = suffix && !strcmp(suffix, "segment");
	switch(cls)
	{
	case TVP_XP3_READ_PSB_FULL:
		return segment ? "psb-segment" : "psb-read";
	case TVP_XP3_READ_AUDIO_STREAM:
		return segment ? "audio-segment" : "audio-read";
	case TVP_XP3_READ_VIDEO_STREAM:
		return segment ? "video-segment" : "video-read";
...
static tjs_uint TVPXP3ReadChunkForClass(tTVPXP3ReadClass cls, bool sequential)
{
	switch(cls)
	{
	case TVP_XP3_READ_PSB_FULL:
		return TVPXP3FlagDisabled("krkr_xp3_psb_large") ? 256 * 1024 : 2 * 1024 * 1024;
	case TVP_XP3_READ_AUDIO_STREAM:
		return TVPXP3FlagDisabled("krkr_xp3_audio_large") ? 256 * 1024 :
			(sequential ? 1024 * 1024 : 512 * 1024);
	case TVP_XP3_READ_VIDEO_STREAM:
		return TVPXP3FlagDisabled("krkr_xp3_video_large") ? 256 * 1024 :
			(sequential ? 2 * 1024 * 1024 : 1024 * 1024);
	case TVP_XP3_READ_SMALL_RANDOM:
	default:
		return 256 * 1024;
	}
...
struct tTVPXP3ReadHintScope
{
	tTVPXP3ReadHintScope(tTVPXP3ReadClass cls, const char *mode,
		const std::string &archive, const std::string &item,
		tjs_uint64 logical_size, tjs_uint64 arc_size,
		tjs_uint request_size, bool sequential)
	{
		if(!TVPXP3FlagDisabled("krkr_xp3_adaptive_read"))
		{
			TVPSetEmscriptenXP3ReadHint(cls, mode, archive.c_str(), item.c_str(),
				logical_size, arc_size, request_size, sequential);
		}
	}
	~tTVPXP3ReadHintScope()
	{
		TVPClearEmscriptenXP3ReadHint();
	}
};
#endif
...
static tTJSBinaryStream * TVPGetCachedArchiveHandle(void * pointer, const ttstr & name)
{
...
#ifdef __EMSCRIPTEN__
	KRKR_LOG_L3("[XP3-HANDLE] get begin archive=%s owner=%p shutdown=%d\n",
		TVPXP3TraceStorageNameUtf8(name).c_str(), pointer,
		TVPArchiveHandleCacheShutdown ? 1 : 0);
#endif
	...
	for(tjs_int i =0; i < TVP_MAX_ARCHIVE_HANDLE_CACHE; i++)
	{
		...
		if(item->Stream && item->Pointer == pointer)
		{
			...
			item->Stream = NULL;
#ifdef __EMSCRIPTEN__
			KRKR_LOG_L3("[XP3-HANDLE] get cache-hit archive=%s owner=%p stream=%p\n",
				TVPXP3TraceStorageNameUtf8(name).c_str(), pointer, (void*)stream);
#endif
			return stream;
		}
	}
...
#ifdef __EMSCRIPTEN__
	KRKR_LOG_L3("[XP3-HANDLE] get opened archive=%s owner=%p stream=%p\n",
		TVPXP3TraceStorageNameUtf8(name).c_str(), pointer, (void*)stream);
#endif
	return stream;
}
...
tTJSBinaryStream * tTVPXP3Archive::CreateStreamByIndex(tjs_uint idx)
{
...
#ifdef __EMSCRIPTEN__
	bool tracePsb = TVPXP3TracePsbName(item.Name);
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-STREAM] by-index begin archive=%s item=%s index=%u org=%llu arc=%llu segs=%zu\n",
			TVPXP3TraceStorageNameUtf8(Name).c_str(),
			TVPXP3TraceStorageNameUtf8(item.Name).c_str(),
			(unsigned)idx,
			(unsigned long long)item.OrgSize,
			(unsigned long long)item.ArcSize,
			item.Segments.size());
	}
#endif
	...
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-STREAM] by-index done archive=%s item=%s index=%u stream=%p out=%p\n",
			TVPXP3TraceStorageNameUtf8(Name).c_str(),
			TVPXP3TraceStorageNameUtf8(item.Name).c_str(),
			(unsigned)idx, (void*)stream, (void*)out);
	}
#endif
	return out;
}
...
public:
	...
	void SetData(unsigned long outsize, tTJSBinaryStream *instream,
		unsigned long insize, tjs_uint read_chunk_size = 64 * 1024)
	{
		// uncompress data
#ifdef __EMSCRIPTEN__
		KRKR_LOG_L3("[XP3-SEG] setdata begin out=%lu in=%lu chunk=%u stream=%p\n",
			outsize, insize, (unsigned)read_chunk_size, (void*)instream);
#endif
		if(read_chunk_size < 64 * 1024) read_chunk_size = 64 * 1024;
		if(read_chunk_size > 4 * 1024 * 1024) read_chunk_size = 4 * 1024 * 1024;
		...
		try
		{
			...
			{
				tjs_uint request =
					(tjs_uint)std::min<unsigned long>(read_chunk_size, insize - totalread);
				tjs_uint read = instream->Read(indata + totalread, request);
#ifdef __EMSCRIPTEN__
				if(totalread == 0 || read == 0 || totalread + read >= insize)
				{
					KRKR_LOG_L3("[XP3-SEG] setdata chunk offset=%lu request=%u got=%u in=%lu\n",
						totalread, (unsigned)request, (unsigned)read, insize);
				}
#endif
				if(read == 0) TVPThrowExceptionMessage(TVPReadError);
				totalread += read;
			}
			...
			Size = outsize;
#ifdef __EMSCRIPTEN__
			KRKR_LOG_L3("[XP3-SEG] setdata done out=%lu in=%lu\n", outsize, insize);
#endif
		}
		catch(...)
		{
...
void tTVPXP3ArchiveStream::EnsureSegment()
{
...
#ifdef __EMSCRIPTEN__
	std::string traceArchive = Owner ? TVPXP3TraceStorageNameUtf8(Owner->GetName()) : "";
	std::string traceItem = Owner ? TVPXP3TraceStorageNameUtf8(Owner->GetName(StorageIndex)) : "";
	std::string lowerItem = Owner ? TVPXP3LowerStorageName(Owner->GetName(StorageIndex)) : "";
	bool tracePsb = Owner && TVPXP3TracePsbName(Owner->GetName(StorageIndex));
	tTVPXP3ReadClass readClass;
	TVPXP3ClassifyStorageName(lowerItem, readClass);
	const char *mode = TVPXP3ModeName(readClass, "segment");
	bool sequentialHint = readClass == TVP_XP3_READ_PSB_FULL || SequentialReadCount >= 2;
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-READ] ensure begin archive=%s item=%s seg=%d opened=%d remain=%llu pos=%llu\n",
			traceArchive.c_str(),
			traceItem.c_str(),
			CurSegmentNum,
			SegmentOpened ? 1 : 0,
			(unsigned long long)SegmentRemain,
...
#ifdef __EMSCRIPTEN__
			tTVPXP3ReadHintScope hintScope(readClass, mode, traceArchive, traceItem,
				OrgSize, CurSegment->ArcSize, (tjs_uint)CurSegment->ArcSize, sequentialHint);
			SegmentData->SetData((tjs_uint)CurSegment->OrgSize,
				Stream, (tjs_uint)CurSegment->ArcSize,
				TVPXP3ReadChunkForClass(readClass, sequentialHint));
#else
			SegmentData->SetData((tjs_uint)CurSegment->OrgSize,
				Stream, (tjs_uint)CurSegment->ArcSize);
#endif
	...
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-READ] ensure done archive=%s item=%s seg=%d compressed=%d remain=%llu pos=%llu\n",
			traceArchive.c_str(),
			traceItem.c_str(),
			CurSegmentNum,
			CurSegment->IsCompressed ? 1 : 0,
			(unsigned long long)SegmentRemain,
			(unsigned long long)CurPos);
	}
#endif
...
tjs_uint TJS_INTF_METHOD tTVPXP3ArchiveStream::Read(void *buffer, tjs_uint read_size)
{
	...
	std::string lowerItem = Owner ? TVPXP3LowerStorageName(Owner->GetName(StorageIndex)) : "";
	bool tracePsb = Owner && TVPXP3TracePsbName(Owner->GetName(StorageIndex));
	bool readIsSequential = CurPos == LastReadEndPos;
	if(readIsSequential) SequentialReadCount++;
	else SequentialReadCount = 0;
	tTVPXP3ReadClass readClass;
	TVPXP3ClassifyStorageName(lowerItem, readClass);
	bool sequentialHint = readClass == TVP_XP3_READ_PSB_FULL || SequentialReadCount >= 2;
	const char *mode = TVPXP3ModeName(readClass, "read");
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-READ] read begin archive=%s item=%s request=%u pos=%llu remain=%llu seq=%u\n",
			traceArchive.c_str(),
			traceItem.c_str(),
			(unsigned)read_size,
			(unsigned long long)CurPos,
			(unsigned long long)SegmentRemain,
			(unsigned)SequentialReadCount);
	}
#endif
	EnsureSegment();
...
#ifdef __EMSCRIPTEN__
			tTVPXP3ReadHintScope hintScope(readClass, mode, traceArchive, traceItem,
				OrgSize, CurSegment->ArcSize, one_size, sequentialHint);
#endif
...
#ifdef __EMSCRIPTEN__
	LastReadEndPos = CurPos;
	if(tracePsb)
	{
		KRKR_LOG_L3("[XP3-READ] read done archive=%s item=%s wrote=%u pos=%llu remain=%llu\n",
			traceArchive.c_str(),
			traceItem.c_str(),
			(unsigned)write_size,
			(unsigned long long)CurPos,
