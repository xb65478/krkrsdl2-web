...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/removeContinuousHandler)
{
...
#ifdef __EMSCRIPTEN__
	{
		...
		if (cn == "KAGWindow" && ot) {
			...
			tTJSVariant cond_var;
			tjs_error cr = ot->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("conductor"), NULL, &cond_var, ot);
			if (TJS_SUCCEEDED(cr) && cond_var.Type() == tvtObject) {
				iTJSDispatch2 *cond = cond_var.AsObjectNoAddRef();
				if (cond) {
					auto getProp = [&](iTJSDispatch2 *obj, const tjs_char *name) -> std::string {
						tTJSVariant v;
						tjs_error r = obj->PropGet(TJS_MEMBERMUSTEXIST, name, NULL, &v, obj);
						if (TJS_SUCCEEDED(r) && v.Type() == tvtString)
							return ttstr(v.GetString()).AsNarrowStdString();
						if (TJS_SUCCEEDED(r) && v.Type() == tvtInteger)
							return std::to_string((long long)v.AsInteger());
						if (TJS_SUCCEEDED(r) && v.Type() == tvtReal)
							return std::to_string(v.AsReal());
						if (TJS_FAILED(r)) return "(n/a)";
						return "(other)";
					};
					if (TVPLogL3()) {
						fprintf(stderr, "[RCH-COND] conductor: curStorage=%s curLabel=%s status=%s\n",
							getProp(cond, TJS_W("curStorage")).c_str(),
							getProp(cond, TJS_W("curLabel")).c_str(),
							getProp(cond, TJS_W("status")).c_str());
						fprintf(stderr, "[RCH-COND2] conductor: waitedByOther=%s interrupted=%s macroLevel=%s\n",
							getProp(cond, TJS_W("waitedByOther")).c_str(),
							getProp(cond, TJS_W("interrupted")).c_str(),
							getProp(cond, TJS_W("macroLevel")).c_str());
					}
				}
			}
			auto getWP = [&](const tjs_char *name) -> std::string {
				tTJSVariant v;
				tjs_error r = ot->PropGet(TJS_MEMBERMUSTEXIST, name, NULL, &v, ot);
				if (TJS_SUCCEEDED(r) && v.Type() == tvtString)
					return ttstr(v.GetString()).AsNarrowStdString();
				if (TJS_SUCCEEDED(r) && v.Type() == tvtInteger)
					return std::to_string((long long)v.AsInteger());
				...
				if (TVPLogL3()) {
					fprintf(stderr, "[RCH-WIN] inSleep=%s sleepCauser=%s transCount=%s clickWaiting=%s\n",
						getWP(TJS_W("inSleep")).c_str(), getWP(TJS_W("sleepCauser")).c_str(),
						getWP(TJS_W("transCount")).c_str(), getWP(TJS_W("clickWaiting")).c_str());
					fprintf(stderr, "[RCH-WIN2] inStable=%s isWaiting=%s waitCount=%s autoMode=%s\n",
						getWP(TJS_W("inStable")).c_str(), getWP(TJS_W("isWaiting")).c_str(),
						getWP(TJS_W("waitCount")).c_str(), getWP(TJS_W("autoMode")).c_str());
					fprintf(stderr, "[RCH-WIN3] currentNum=%s currentPage=%s currentLabel=%s\n",
						getWP(TJS_W("currentNum")).c_str(), getWP(TJS_W("currentPage")).c_str(),
						getWP(TJS_W("currentLabel")).c_str());
				}
			} else {
				if (TVPLogL3()) {
					fprintf(stderr, "[RCH-STACK] removeContinuousHandler class=%s\n", cn.c_str());
				}
			}
		}
	#endif
...
#endif
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_METHOD_DECL(addFont)
{
	return TJS_S_OK;
}
TJS_END_NATIVE_STATIC_METHOD_DECL(addFont)
//----------------------------------------------------------------------
