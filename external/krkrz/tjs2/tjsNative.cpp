	...
	iTJSDispatch2 *objthis)
{
	...
	if(membername) {
		...
		if( hr == TJS_E_MEMBERNOTFOUND ) {
			KRKR_LOG_L2("[TJS-NF] native class=%s method=%s\n",
				ClassName.AsNarrowStdString().c_str(),
				ttstr(membername).AsNarrowStdString().c_str());
		}
		return hr;
	}
...
#if defined(__EMSCRIPTEN__)
				{
					...
					if(nm) {
						...
						if(isContains || isSkip) {
							KRKR_LOG_L3("[DIAG-NCM] NativeClass copying %s to %p type=%d\n",
								isContains ? "contains" : "skipToSync", (void*)Dest, (int)val.Type());
							fflush(stderr);
							// Verify it's actually on the instance
							tTJSVariant check;
							tjs_error hr2 = Dest->PropGet(TJS_MEMBERMUSTEXIST, nm, NULL, &check, Dest);
							KRKR_LOG_L3("[DIAG-NCM] Verify after copy: hr=%d type=%d\n", (int)hr2, (int)check.Type());
							fflush(stderr);
						}
					}
				}
