...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func.name*/saveStruct)
{
	...
	if( TJS_strchr(mode.c_str(), TJS_W('b')) != NULL ) {
#ifdef __EMSCRIPTEN__
		KRKR_LOG_L2("[SYSVAR-SAVE] dict-saveStruct begin path=%s mode=%s binary=1\n",
			name.AsNarrowStdString().c_str(), mode.AsNarrowStdString().c_str());
#endif
		...
		try {
			...
			delete stream;
#ifdef __EMSCRIPTEN__
			KRKR_LOG_L2("[SYSVAR-SAVE] dict-saveStruct done path=%s binary=1\n",
				name.AsNarrowStdString().c_str());
#endif
		} else {
#ifdef __EMSCRIPTEN__
			KRKR_LOG_L2("[SYSVAR-SAVE] dict-saveStruct begin path=%s mode=%s binary=0\n",
				name.AsNarrowStdString().c_str(), mode.AsNarrowStdString().c_str());
#endif
		...
		catch(...)
		{
			...
			stream->Destruct();
#ifdef __EMSCRIPTEN__
			KRKR_LOG_L2("[SYSVAR-SAVE] dict-saveStruct done path=%s binary=0\n",
				name.AsNarrowStdString().c_str());
#endif
		}
...
{
	fprintf(stderr, "[DICT_CLONE] called\n");
	TJS_GET_NATIVE_INSTANCE(/* var. name */ni, /* var. type */tTJSDictionaryNI);
	if(!ni->IsValid()) return TJS_E_INVALIDOBJECT;

	iTJSDispatch2 *newdict = TJSCreateDictionaryObject();
	try
	{
		tTJSVariant src(objthis, objthis);
		tTJSVariant *psrc = &src;
		newdict->FuncCall(0, TJS_W("assign"), NULL, NULL, 1, &psrc, newdict);
	}
	catch(...)
	{
		newdict->Release();
	...
	if(result) *result = tTJSVariant(newdict, newdict);
	newdict->Release();
	return TJS_S_OK;
}
