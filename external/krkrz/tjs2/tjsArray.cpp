...
public:
	bool operator () (const tTJSVariant &lhs, const tTJSVariant &rhs) const
	{
		if(lhs.Type() == tvtString && rhs.Type() == tvtString)
		{
			...
			ltmp.tonumber();
			rtmp.tonumber();
			return (ltmp < rtmp).operator bool(); // explicit bool conversion
		}
		return (lhs < rhs).operator bool(); // explicit bool conversion
	}
};
...
TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */clone)
{
	...
	iTJSDispatch2 *newarray = TJSCreateArrayObject();
	try
	{
		tTJSVariant src(objthis, objthis);
		tTJSVariant *psrc = &src;
		newarray->FuncCall(0, TJS_W("assign"), NULL, NULL, 1, &psrc, newarray);
	}
	catch(...)
	{
		newarray->Release();
	...
	if(result) *result = tTJSVariant(newarray, newarray);
	newarray->Release();
	return TJS_S_OK;
}
