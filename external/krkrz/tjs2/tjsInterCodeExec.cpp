...
// utility functions
//---------------------------------------------------------------------------
static bool TJSShouldLogThrow(const char *message)
{
#ifdef __EMSCRIPTEN__
	if(message && !strcmp(message, "Out of Rectangle"))
	{
		static int out_of_rectangle_logs = 0;
		if(out_of_rectangle_logs < 3)
		{
			out_of_rectangle_logs++;
			return true;
		}
		if(out_of_rectangle_logs == 3)
		{
			out_of_rectangle_logs++;
			fprintf(stderr,
				"[TJS-THROW] Out of Rectangle repeated; suppressing further identical throw logs\n");
		}
		return false;
	}
#endif
...
static bool TJSShouldLogThrow(const std::string &message)
{
	return TJSShouldLogThrow(message.c_str());
}
...
#define DEBUGGER_EXCEPTION_HOOK
	...
	{
		DEBUGGER_EXCEPTION_HOOK;
		std::string message = e.GetMessage().AsNarrowStdString();
		if(TJSShouldLogThrow(message))
			fprintf(stderr, "[TJS-THROW] %s at %s\n",
				message.c_str(),
				GetPositionDescriptionString((tjs_int)(codesave - CodeArea)).AsNarrowStdString().c_str());
	...
	{
		DEBUGGER_EXCEPTION_HOOK;
		std::string message = ttstr(text).AsNarrowStdString();
		if(TJSShouldLogThrow(message))
			fprintf(stderr, "[TJS-THROW] %s at %s\n",
				message.c_str(),
				GetPositionDescriptionString((tjs_int)(codesave - CodeArea)).AsNarrowStdString().c_str());
	...
	const tjs_int32 *code, tjs_uint32 flags)
{
	...
	{
		const tjs_char *nm = name->GetString();
		if(nm) {
			bool isDt = (nm[0]=='d' && nm[1]=='t' && nm[2]=='D' && nm[3]=='r' && nm[4]=='a' && nm[5]=='w');
			bool isContains = (nm[0]=='c' && nm[1]=='o' && nm[2]=='n' && nm[3]=='t' && nm[4]=='a' && nm[5]=='i' && nm[6]=='n' && nm[7]=='s' && nm[8]==0);
			bool isSkip = (nm[0]=='s' && nm[1]=='k' && nm[2]=='i' && nm[3]=='p' && nm[4]=='T' && nm[5]=='o');
				if(TVPLogL3() && (isDt || isContains || isSkip)) {
				fprintf(stderr, "[DIAG-GPD] GetPropertyDirect '%s' Object=%p ObjThis=%p ra_code2_type=%d\n",
					isDt ? "dtDrawDib" : (isContains ? "contains" : "skipToSync"),
					(void*)clo.Object, (void*)clo.ObjThis, (int)type);
				fflush(stderr);
				tTJSVariant testVal;
				iTJSDispatch2 *target = clo.ObjThis ? clo.ObjThis : ra[-1].AsObjectNoAddRef();
				tjs_error testHr = target->PropGet(TJS_MEMBERMUSTEXIST, nm, NULL, &testVal, target);
				fprintf(stderr, "[DIAG-GPD] Direct PropGet on target=%p: hr=%d\n", (void*)target, (int)testHr);
				if(clo.Object != target) {
					tjs_error testHr2 = clo.Object->PropGet(TJS_MEMBERMUSTEXIST, nm, NULL, &testVal, target);
					fprintf(stderr, "[DIAG-GPD] PropGet on Object=%p: hr=%d\n", (void*)clo.Object, (int)testHr2);
				}
				fflush(stderr);
			}
		}
	...
	if(TJS_FAILED(hr)) {
		const tjs_char *nm = name->GetString();
		if(nm && TJS_strcmp(nm, TJS_W("dtDrawDib")) == 0) {
			tTJSVariant* result_var = TJS_GET_VM_REG_ADDR(ra, code[1]);
			if(result_var) *result_var = (tjs_int)1;
			hr = TJS_S_OK;
		}
	}
#endif
		...
		else
		{
...
#if defined(__EMSCRIPTEN__)
			{
				...
				if(nm) {
					bool isContains = (nm[0]=='c' && nm[1]=='o' && nm[2]=='n' && nm[3]=='t' && nm[4]=='a' && nm[5]=='i' && nm[6]=='n' && nm[7]=='s' && nm[8]==0);
					bool isSkip = (nm[0]=='s' && nm[1]=='k' && nm[2]=='i' && nm[3]=='p' && nm[4]=='T' && nm[5]=='o');
					if(isContains || isSkip) {
						iTJSDispatch2 *target = clo.ObjThis ? clo.ObjThis : ra[-1].AsObjectNoAddRef();
						KRKR_LOG_L3("[DIAG-CFD] CallFuncDirect '%s' Object=%p ObjThis=%p target=%p\n",
							isContains ? "contains" : "skipToSync",
							(void*)clo.Object, (void*)clo.ObjThis, (void*)target);
						tTJSVariant testVal;
						tjs_error testHr = clo.Object->PropGet(TJS_MEMBERMUSTEXIST, nm, NULL, &testVal, target);
						KRKR_LOG_L3("[DIAG-CFD] PropGet on Object: hr=%d\n", (int)testHr);
						if(target != clo.Object) {
							testHr = target->PropGet(TJS_MEMBERMUSTEXIST, nm, NULL, &testVal, target);
							KRKR_LOG_L3("[DIAG-CFD] PropGet on target: hr=%d\n", (int)testHr);
						}
						fflush(stderr);
					}
				}
...
#if defined(__EMSCRIPTEN__)
				{
					const tjs_char *nm = param[0]->GetString();
					if(nm) {
						bool isContains = (nm[0]=='c' && nm[1]=='o' && nm[2]=='n' && nm[3]=='t' && nm[4]=='a' && nm[5]=='i' && nm[6]=='n' && nm[7]=='s' && nm[8]==0);
						bool isSkip = (nm[0]=='s' && nm[1]=='k' && nm[2]=='i' && nm[3]=='p' && nm[4]=='T' && nm[5]=='o' && nm[6]=='S' && nm[7]=='y' && nm[8]=='n' && nm[9]=='c' && nm[10]==0);
						if(isContains || isSkip) {
							fprintf(stderr, "[DIAG-REG] VM_REGMEMBER copying %s to %p type=%d\n",
								isContains ? "contains" : "skipToSync", (void*)Dest, (int)val.Type());
							fflush(stderr);
						}
					}
				}
