	...
	tjs_uint32 *hint)
{
	if(!name) return NULL;
#ifdef __EMSCRIPTEN__
	if(!tvpIsValidWasmPtr(Symbols)) return NULL;
#endif
	...
	tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
{
	...
	if(membername &&
	   (TJS_strcmp(membername, TJS_W("enqueueFastTag")) == 0 ||
	    TJS_strcmp(membername, TJS_W("pushFastTag")) == 0 ||
	    TJS_strcmp(membername, TJS_W("enqueueTag")) == 0 ||
	    TJS_strcmp(membername, TJS_W("addFastTag")) == 0))
	{
		KRKR_LOG_L3("[TJS-CALL] method=%s numparams=%d\n",
			ttstr(membername).AsNarrowStdString().c_str(), (int)numparams);
	}
	...
	if(!data)
	{
...
#if defined(__EMSCRIPTEN__)
		// Provide a generic clone() fallback for scriptsEx.dll compatibility
		if(membername && TJS_strcmp(membername, TJS_W("clone")) == 0)
		{
			if(objthis && result)
			{
				// Try CreateNew first (works for native classes)
				iTJSDispatch2 *newobj = NULL;
				tjs_error hr = objthis->CreateNew(0, NULL, NULL, &newobj, 0, NULL, objthis);
				if(TJS_SUCCEEDED(hr) && newobj)
				{
					tTJSVariant src(objthis, objthis);
					tTJSVariant *psrc = &src;
					newobj->FuncCall(0, TJS_W("assign"), NULL, NULL, 1, &psrc, newobj);
					*result = tTJSVariant(newobj, newobj);
					newobj->Release();
					return TJS_S_OK;
				}
				if(newobj) newobj->Release();

				// Fallback: clone into a Dictionary by enumerating members
				iTJSDispatch2 *dict = TJSCreateDictionaryObject();
				if(dict)
				{
					struct CloneCallback : public tTJSDispatch {
						iTJSDispatch2 *dest;
						iTJSDispatch2 *src;
						CloneCallback(iTJSDispatch2 *d, iTJSDispatch2 *s) : dest(d), src(s) {}
						tjs_error TJS_INTF_METHOD FuncCall(tjs_uint32 flag,
							...
							if(numparams >= 3) {
								...
								tjs_int flags = (tjs_int)*param[1];
								if(name && !(flags & TJS_STATICMEMBER)) {
									tTJSVariant val;
									if(TJS_SUCCEEDED(src->PropGet(TJS_MEMBERMUSTEXIST|TJS_IGNOREPROP, name, NULL, &val, src))) {
										dest->PropSet(TJS_MEMBERENSURE, name, NULL, &val, dest);
									}
								}
							}
							return TJS_S_OK;
					...
					CloneCallback *cb = new CloneCallback(dict, objthis);
					tTJSVariantClosure clo(cb, NULL);
					objthis->EnumMembers(TJS_IGNOREPROP, &clo, objthis);
					cb->Release();
		...
		// Provide contains() fallback for Layer compatibility
		if(membername && TJS_strcmp(membername, TJS_W("contains")) == 0)
		{
			if(numparams >= 2 && result)
			{
				tjs_int x = (tjs_int)*param[0];
				tjs_int y = (tjs_int)*param[1];
				tTJSVariant w_val, h_val;
				tjs_error hr_w = TJS_E_MEMBERNOTFOUND, hr_h = TJS_E_MEMBERNOTFOUND;
				if(objthis) {
					hr_w = objthis->PropGet(0, TJS_W("width"), NULL, &w_val, objthis);
					hr_h = objthis->PropGet(0, TJS_W("height"), NULL, &h_val, objthis);
				}
				if(TJS_SUCCEEDED(hr_w) && TJS_SUCCEEDED(hr_h)) {
					tjs_int w = (tjs_int)w_val;
					tjs_int h = (tjs_int)h_val;
					*result = (tjs_int)(x >= 0 && y >= 0 && x < w && y < h);
				} else {
					*result = (tjs_int)0;
				}
				return TJS_S_OK;
		...
		// Provide skipToSync() fallback for MotionPlayer compatibility
		if(membername && TJS_strcmp(membername, TJS_W("skipToSync")) == 0)
		{
			return TJS_S_OK;
		}
		// Provide playBuffer() fallback for bgm.tjs compatibility
		if(membername && TJS_strcmp(membername, TJS_W("playBuffer")) == 0)
		{
			return TJS_S_OK;
		}
#endif
	...
	iTJSDispatch2 *objthis)
{
	...
	if(!data) {
#ifdef __EMSCRIPTEN__
		if(membername && (TJS_strcmp(membername, TJS_W("dtDrawDib")) == 0 || TJS_strcmp(membername, TJS_W("dtDBGDI")) == 0 || TJS_strcmp(membername, TJS_W("dtDBDD")) == 0 || TJS_strcmp(membername, TJS_W("dtDBD3D")) == 0 || TJS_strcmp(membername, TJS_W("dtNone")) == 0)) {
