...
{
	TVPAddLog(ttstr(TJS_W("[DEBUG] tTVPLayerManager created: ")) + ttstr((tjs_int)(size_t)this));
	RefCount = 1;
...
{
	TVPAddLog(ttstr(TJS_W("[DEBUG] tTVPLayerManager destroyed: ")) + ttstr((tjs_int)(size_t)this));
	for(size_t i = 0; i < DeferredInvalidateQueue.size(); i++)
		if(DeferredInvalidateQueue[i].Owner) DeferredInvalidateQueue[i].Owner->Release();
	DeferredInvalidateQueue.clear();
	if(DrawBuffer) delete DrawBuffer;
}
//---------------------------------------------------------------------------
void tTVPLayerManager::DeferInvalidate(tTJSNI_BaseLayer* layer)
{
	if(!layer) return;
	for(size_t i = 0; i < DeferredInvalidateQueue.size(); i++)
		if(DeferredInvalidateQueue[i].Layer == layer) return;

	iTJSDispatch2 *owner = layer->GetOwnerNoAddRef();
	if(owner) owner->AddRef();

	tDeferredInvalidateItem item;
	item.Layer = layer;
	item.Owner = owner;
	DeferredInvalidateQueue.push_back(item);
}
//---------------------------------------------------------------------------
void tTVPLayerManager::DrainDeferredInvalidates()
{
	while(!DeferredInvalidateQueue.empty())
	{
		std::vector<tDeferredInvalidateItem> queue;
		queue.swap(DeferredInvalidateQueue);
		for(size_t i = 0; i < queue.size(); i++)
		{
			if(queue[i].Layer) queue[i].Layer->Invalidate();
			if(queue[i].Owner) queue[i].Owner->Release();
		}
	}
}
//---------------------------------------------------------------------------
...
void tTVPLayerManager::PrimaryClick(tjs_int x, tjs_int y)
{
...
#ifdef __EMSCRIPTEN__
	{
		...
		log_count++;
		if (TVPLogL3() && log_count <= 120) fprintf(stderr, "[INPUT-LAYER] click primary=(%d,%d) hit=%p name=%s capture=%p\n",
			(int)x, (int)y, (void*)l, l ? l->GetName().AsNarrowStdString().c_str() : "-",
			(void*)CaptureOwner);
	}
#endif
	if(l && CaptureOwner == l)
	...
	tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
		if (TVPLogL3() && log_count <= 120) fprintf(stderr, "[INPUT-LAYER] down primary=(%d,%d) hit=%p name=%s capture=%p mb=%d flags=%u\n",
			(int)x, (int)y, (void*)l, l ? l->GetName().AsNarrowStdString().c_str() : "-",
			(void*)CaptureOwner, (int)mb, (unsigned)flags);
	}
#endif
	if(l)
	{
	...
	tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
		if (TVPLogL3() && log_count <= 120) fprintf(stderr, "[INPUT-LAYER] up primary=(%d,%d) hit=%p name=%s capture=%p mb=%d flags=%u\n",
			(int)x, (int)y, (void*)l, l ? l->GetName().AsNarrowStdString().c_str() : "-",
			(void*)CaptureOwner, (int)mb, (unsigned)flags);
	}
#endif
...
void tTVPLayerManager::PrimaryMouseMove(tjs_int x, tjs_int y, tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
		if (TVPLogL3() && log_count <= 120) fprintf(stderr, "[INPUT-LAYER] move primary=(%d,%d) hit=%p name=%s capture=%p flags=%u changed=%d\n",
			(int)x, (int)y, (void*)l, l ? l->GetName().AsNarrowStdString().c_str() : "-",
			(void*)CaptureOwner, (unsigned)flags, poschanged ? 1 : 0);
	}
#endif
...
void tTVPLayerManager::AddUpdateRegion(const tTVPComplexRect &rects)
{
	...
	if(UpdateRegion.GetCount() > TVP_UPDATE_UNITE_LIMIT)
		UpdateRegion.Unite();
#ifdef __EMSCRIPTEN__
	static int aur_count = 0;
	aur_count++;
	if (TVPLogL3() && (aur_count <= 20 || aur_count % 500 == 0))
		fprintf(stderr, "[DIAG-AUR] AddUpdateRegion(complex) #%d count=%d\n", aur_count, (int)rects.GetCount());
#endif
...
#ifdef __EMSCRIPTEN__
	static int aur2_count = 0;
	aur2_count++;
	if (TVPLogL3() && (aur2_count <= 20 || aur2_count % 500 == 0))
		fprintf(stderr, "[DIAG-AUR] AddUpdateRegion(rect) #%d rect=(%d,%d,%d,%d)\n",
			aur2_count, (int)rect.left, (int)rect.top, (int)rect.right, (int)rect.bottom);
#endif
	NotifyWindowInvalidation();
}
//---------------------------------------------------------------------------
