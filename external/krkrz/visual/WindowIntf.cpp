...
void tTJSNI_BaseWindow::OnClick(tjs_int x, tjs_int y)
{
...
#ifdef __EMSCRIPTEN__
	{
		...
		log_count++;
			if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-WINDOW] onClick x=%d y=%d owner=%p\n", (int)x, (int)y, (void*)Owner);
	}
#endif
	if(Owner)
	{
	...
	tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
			if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-WINDOW] onMouseDown x=%d y=%d mb=%d flags=%u owner=%p\n", (int)x, (int)y, (int)mb, (unsigned)flags, (void*)Owner);
	}
#endif
	if(Owner)
	{
	...
	tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
			if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-WINDOW] onMouseUp x=%d y=%d mb=%d flags=%u owner=%p\n", (int)x, (int)y, (int)mb, (unsigned)flags, (void*)Owner);
	}
#endif
	if(Owner)
	{
...
{
	if( DrawDevice ) {
		KRKR_LOG_L3("[UC] DrawDevice=%p\n", (void*)DrawDevice);
		fflush(stderr);
		DrawDevice->Update();
		KRKR_LOG_L3("[UC] Update done, WaitVSync=%d\n", (int)WaitVSync);
		fflush(stderr);
		if( !WaitVSync ) DrawDevice->Show();
		KRKR_LOG_L3("[UC] Show done\n");
		fflush(stderr);
 		EndUpdate();
	}
}
