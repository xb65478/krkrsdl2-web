...
//---------------------------------------------------------------------------
// Bounds-checking helper for pixel write operations (debug)
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
static inline void TVPCheckPixelWriteBounds(
    const void* write_ptr, tjs_int write_bytes,
    const void* buf_start, tjs_uint buf_size,
    const char* func_name, tjs_int y, tjs_int x)
{
    uintptr_t w_start = (uintptr_t)write_ptr;
    uintptr_t w_end = w_start + write_bytes;
    uintptr_t b_start = (uintptr_t)buf_start;
    uintptr_t b_end = b_start + buf_size;
    if(w_start < b_start || w_end > b_end) {
        fprintf(stderr, "[OOB] %s: write [%p..%p) outside buf [%p..%p) y=%d x=%d overshoot=%d\n",
            func_name, (void*)w_start, (void*)w_end,
            (void*)b_start, (void*)b_end,
            y, x, (int)(w_end - b_end));
        fflush(stderr);
    }
}
#define TVP_CHECK_WRITE(ptr, bytes, buf, bufsz, fn, y, x) \
    TVPCheckPixelWriteBounds(ptr, bytes, buf, bufsz, fn, y, x)
#else
#define TVP_CHECK_WRITE(ptr, bytes, buf, bufsz, fn, y, x) ((void)0)
#endif
...
bool tTVPBaseBitmap::Fill(tTVPRect rect, tjs_uint32 value)
{
	...
	if(!IsIndependent())
	{
        ...
        // Check bitmap buffer sentinels
        {
            const void* bits = GetBitmap()->GetBits();
            if(bits) tTVPBitmapBitsAlloc::CheckSentinels(const_cast<void*>(bits), "Fill");
        }
...
void tTVPBaseBitmap::PartialFill(const PartialFillParam *param)
{
	...
	if(param->is32bpp)
	{
...
#ifdef __EMSCRIPTEN__
		const void* _bcheck_base = param->self->GetBitmap()->GetBits();
		tjs_uint _bcheck_size = param->self->GetBitmap()->GetBitmapInfomation()->GetImageSize();
#endif
...
#endif
		{
			...
			{
				tjs_uint32 * p = (tjs_uint32*)sc + x;
				TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialFill", param->y, x);
...
bool tTVPBaseBitmap::FillColor(tTVPRect rect, tjs_uint32 color, tjs_int opa)
{
	...
	tjs_uint8 *dest = (tjs_uint8*)GetScanLineForWrite(0);
        ...
        {
            const void* bits = GetBitmap()->GetBits();
            if(bits) tTVPBitmapBitsAlloc::CheckSentinels(const_cast<void*>(bits), "FillColor");
        }
  ...
  tjs_uint _bcheck_size = param->self->GetBitmap()->GetBitmapInfomation()->GetImageSize();
	...
	if(opa == 255)
	{
		...
		while(height--)
		{
			...
			TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialFillColor(opa255)", param->y, left);
	...
	else
	{
		...
		{
			tjs_uint32 * p = (tjs_uint32*)sc + left;
			TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialFillColor(alpha)", param->y, left);
	...
	bool additive)
{
	...
	tjs_uint8 *dest = (tjs_uint8*)GetScanLineForWrite(0);
        ...
        {
            const void* bits = GetBitmap()->GetBits();
            if(bits) tTVPBitmapBitsAlloc::CheckSentinels(const_cast<void*>(bits), "BlendColor");
        }
...
void tTVPBaseBitmap::PartialBlendColor(const PartialBlendColorParam *param)
{
  ...
  tjs_uint _bcheck_size = param->self->GetBitmap()->GetBitmapInfomation()->GetImageSize();
	...
	if(opa == 255)
	{
		...
		{
			tjs_uint32 * p = (tjs_uint32*)sc + left;
			TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialBlendColor(opa255)", param->y, left);
	...
	else
	{
		...
		if(!additive)
		{
			...
			{
				tjs_uint32 * p = (tjs_uint32*)sc + left;
				TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialBlendColor(alpha)", param->y, left);
		...
		else
		{
			...
			{
				tjs_uint32 * p = (tjs_uint32*)sc + left;
				TVP_CHECK_WRITE(p, width * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialBlendColor(additive)", param->y, left);
...
{
	if(!ref) return false;
	if(!IsValid() || !ref->IsValid()) return false;
	...
	tjs_int pixelsize = (Is32BPP()?sizeof(tjs_uint32):sizeof(tjs_uint8));
        ...
        {
            const void* bits = GetBitmap()->GetBits();
            if(bits) tTVPBitmapBitsAlloc::CheckSentinels(const_cast<void*>(bits), "CopyRect");
        }
...
void tTVPBaseBitmap::PartialCopyRect(const PartialCopyRectParam *param)
{
	...
	if(backwardCopy)
	{
		...
		TVP_CHECK_WRITE(dest, wbytes, _bcheck_base, _bcheck_size, "PartialCopyRect(bwd:first)", param->dy + param->h - 1, param->dx);
		TVP_CHECK_WRITE(dest - dpitch * (h - 1), wbytes, _bcheck_base, _bcheck_size, "PartialCopyRect(bwd:last)", param->dy, param->dx);
	...
	else
	{
		...
		TVP_CHECK_WRITE(dest, wbytes, _bcheck_base, _bcheck_size, "PartialCopyRect(fwd:first)", param->dy, param->dx);
		TVP_CHECK_WRITE(dest + dpitch * (h - 1), wbytes, _bcheck_base, _bcheck_size, "PartialCopyRect(fwd:last)", param->dy + h - 1, param->dx);
		...
		tTVPRect refrect, tTVPBBBltMethod method, tjs_int opa, bool hda)
{
	...
	tjs_int h = refrect.get_height();
        ...
        {
            const void* bits = GetBitmap()->GetBits();
            if(bits) tTVPBitmapBitsAlloc::CheckSentinels(const_cast<void*>(bits), "Blt");
        }
...
void tTVPBaseBitmap::PartialBlt(const PartialBltParam *param)
{
...
#ifdef __EMSCRIPTEN__
  {
    const void* _bcheck_base = param->self->GetBitmap()->GetBits();
    tjs_uint _bcheck_size = param->self->GetBitmap()->GetBitmapInfomation()->GetImageSize();
    TVP_CHECK_WRITE(dest, w * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialBlt(first)", dy, dx);
    if(h > 1) TVP_CHECK_WRITE(dest + dpitch * (h - 1), w * sizeof(tjs_uint32), _bcheck_base, _bcheck_size, "PartialBlt(last)", dy + h - 1, dx);
  }
#endif
