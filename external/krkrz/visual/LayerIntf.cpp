...
static inline bool tvpIsValidWasmPtr(const void* p) {
	if (!p) return false;
	uintptr_t addr = (uintptr_t)p;
	uintptr_t heapSize = (uintptr_t)(__builtin_wasm_memory_size(0)) * 65536;
	return addr < heapSize;
}
#else
static inline bool tvpIsValidWasmPtr(const void* p) { return p != nullptr; }
#endif
#include <cmath>
...
#ifdef __EMSCRIPTEN__
#define TVP_CHECK_MANAGER(op_name) \
    do { \
        if(Manager != _tvp_mgr_saved && _tvp_mgr_saved != nullptr) { \
            fprintf(stderr, "[STOMP] Manager changed during %s: was=%p now=%p this=%p MainImage=%p\n", \
                op_name, _tvp_mgr_saved, (void*)Manager, (void*)this, (void*)MainImage); \
            fflush(stderr); \
        } \
        _tvp_mgr_saved = Manager; \
    } while(0)
#else
#define TVP_CHECK_MANAGER(op_name) ((void)0)
#endif
...
#define TVP_LAYER_FOR_EACH_CHILD_BEGIN(varname)              \
	{ \
		...
		{                                                                  \
			...
			if(!tvpIsValidWasmPtr(varname)) continue; \
			if(varname->LayerMagic != 0xDEADBEEF) continue; \
			if(varname->Shutdown) continue;
               
#define TVP_LAYER_FOR_EACH_CHILD_END \
		} \
		ChildrenArrayValid = false; \
		ChildrenOrderIndexValid = false; \
		if(tvpIsValidWasmPtr(Manager) && !Shutdown) Manager->InvalidateOverallIndex(); \
	}
//---------------------------------------------------------------------------
...
{
	LayerMagic = 0;
	if(Parent && tvpIsValidWasmPtr(Parent))
	{
		...
		iTJSDispatch2 *tjs_obj)
{
	...
	if(lay)
	{
		...
		TVPAddLog(ttstr(TJS_W("[DEBUG] tTJSNI_BaseLayer::Construct (child) Manager assigned: ")) + ttstr((tjs_int)(size_t)Manager) + TJS_W(" to layer: ") + ttstr((tjs_int)(size_t)this));
	}
	...
	if(!lay)
	{
		...
		TVPAddLog(ttstr(TJS_W("[DEBUG] tTJSNI_BaseLayer::Construct (primary) Manager created: ")) + ttstr((tjs_int)(size_t)Manager) + TJS_W(" to layer: ") + ttstr((tjs_int)(size_t)this));
...
{
	if(Manager && Manager->IsInRendering())
	{
		Shutdown = true;
		Manager->DeferInvalidate(this);
		return;
	}
	...
	{
		if(TVPLogL3())
			TVPAddLog(ttstr(TJS_W("[DEBUG] tTJSNI_BaseLayer::Invalidate Manager clearing: ")) + ttstr((tjs_int)(size_t)Manager) + TJS_W(" from layer: ") + ttstr((tjs_int)(size_t)this));
...
{
	#ifdef __EMSCRIPTEN__
	KRKR_LOG_L3("[LOAD-IMG] LoadImages called: name=%s colorkey=0x%08x\n",
		name.AsNarrowStdString().c_str(), (unsigned)colorkey);
#endif
	...
	if (TVPLogL3())
	{
		tjs_int iw = MainImage->GetWidth(), ih = MainImage->GetHeight();
		tjs_uint32 firstpx = 0, midpx = 0, midpx2 = 0;
		int nonzero = 0;
		if (iw > 0 && ih > 0) {
			const void *sl0 = MainImage->GetScanLine(0);
			if (sl0) firstpx = *(const tjs_uint32*)sl0;
			const void *slm = MainImage->GetScanLine(ih / 2);
			if (slm) {
				midpx = ((const tjs_uint32*)slm)[iw / 2];
				midpx2 = ((const tjs_uint32*)slm)[iw / 4];
				const tjs_uint32 *row = (const tjs_uint32*)slm;
				for (int i = 0; i < iw; i++) if (row[i] != 0) nonzero++;
			}
		}
		fprintf(stderr, "[LOAD-IMG] Result: %dx%d firstpx=%08x midpx=%08x midpx_q=%08x mid_nonzero=%d/%d\n",
			(int)iw, (int)ih, (unsigned)firstpx, (unsigned)midpx, (unsigned)midpx2, nonzero, (int)iw);
	}
#endif
	try
	{
...
bool tTJSNI_BaseLayer::HitTestNoVisibleCheck(tjs_int x, tjs_int y)
{
	...
	if(Owner && !Shutdown)
	{
		...
		res = OnHitTest_Work;
#ifdef __EMSCRIPTEN__
		if (x >= 30 && x <= 630 && y >= 80 && y <= 205) {
			KRKR_LOG_L3("[HITTEST-CALL] layer=%s x=%d y=%d result=%d\n",
				GetName().AsNarrowStdString().c_str(), (int)x, (int)y, res ? 1 : 0);
		}
#endif
	}
	...
	const tTJSNI_BaseLayer *except, bool get_disabled)
{
...
#ifdef __EMSCRIPTEN__
	tjs_int absleft = Rect.left;
	tjs_int abstop = Rect.top;
	for(tTJSNI_BaseLayer *p = Parent; p; p = p->Parent)
	{
		absleft += p->Rect.left;
		abstop += p->Rect.top;
	}
	tjs_int primary_x = x + absleft;
	tjs_int primary_y = y + abstop;
	bool trace_slot = primary_x >= 300 && primary_x <= 360 && primary_y >= 120 && primary_y <= 165;
	if(trace_slot && TVPLogL3())
	{
		...
		{
			fprintf(stderr,
				"[HITTEST-TREE] layer=%s visible=%d owner=%p primary=(%d,%d) local=(%d,%d) rect=(%d,%d,%d,%d) childCount=%d\n",
				GetName().AsNarrowStdString().c_str(), Visible ? 1 : 0, (void*)Owner,
				(int)primary_x, (int)primary_y, (int)x, (int)y,
				(int)absleft, (int)abstop, (int)Rect.get_width(), (int)Rect.get_height(),
				(int)Children.GetSafeLockedObjectCount());
		}
	}
#endif
	...
	bool native_hit = _HitTestNoVisibleCheck(ox, oy);
	if(HitTestNoVisibleCheck(ox, oy))
	{
		if(!get_disabled && IsDisabledByMode())
		{
			return false;
		}
		if(!get_disabled && !GetNodeEnabled())
		{
			if(!native_hit) return false;
			*lay = NULL;
...
#ifdef __EMSCRIPTEN__
	static int draw_text_log_count = 0;
	if(TVPLogL3() && text.GetLen() > 0 && draw_text_log_count < 200)
	{
		std::string narrow = text.AsNarrowStdString();
		if(narrow.find_first_not_of(" \t\r\n") != std::string::npos)
		{
			draw_text_log_count++;
			std::string preview = narrow.substr(0, 120);
			fprintf(stderr,
				"[DRAW-TEXT] #%d layer=%p pos=(%d,%d) len=%d opa=%d aa=%d text=%s\n",
				draw_text_log_count, (void*)this, (int)x, (int)y, (int)text.GetLen(),
				(int)opa, (int)aa, preview.c_str());
		}
	}
#endif
	...
	tTVPRect &cr)
{
	...
	{
		if(MainImage && MainImage->IsValid() && MainImage->GetWidth() > 0 && MainImage->GetHeight() > 0)
		{
	...
	const tTVPRect &srcrect)
{
	...
	else
	{
		...
		else
		{
			...
			if(dest) dest->Fill(tTVPRect(destx, desty,
...
{
	static int drawCrashDebug = 0;
	drawCrashDebug++;
	if(drawCrashDebug <= 50) {
		fprintf(stderr, "[DRAW#%d] this=%p sd=%d mgr=%p mainimg=%p nch=%d",
			drawCrashDebug, (void*)this,
			tvpIsValidWasmPtr(this) ? (int)Shutdown : -1,
			tvpIsValidWasmPtr(this) ? (void*)Manager : nullptr,
			tvpIsValidWasmPtr(this) ? (void*)MainImage : nullptr,
			tvpIsValidWasmPtr(this) ? Children.GetCount() : -1);
		if(tvpIsValidWasmPtr(this)) {
			int nc = Children.GetCount();
			for(int ci = 0; ci < nc && ci < 6; ci++) {
				tTJSNI_BaseLayer* ch = Children[ci];
				fprintf(stderr, " ch[%d]=%p", ci, (void*)ch);
				if(ch && tvpIsValidWasmPtr(ch)) {
					fprintf(stderr, "(m=%x,sd=%d)", ch->LayerMagic, (int)ch->Shutdown);
				}
			}
		}
		fprintf(stderr, "\n");
	...
	if(!tvpIsValidWasmPtr(this) || LayerMagic != 0xDEADBEEF) return;
	if(Shutdown) return;
	if(!tvpIsValidWasmPtr(Manager)) return;
	void* _tvp_mgr_saved = Manager;
		...
		!(InTransition && !TransWithChildren && DivisibleTransHandler))
	{
		...
		while(it.Step())
		{
			...
			if(MainImage != NULL)
			{
				...
				TVP_CHECK_MANAGER("Draw:CopySelf");
			}
			...
			TVP_LAYER_FOR_EACH_CHILD_BEGIN(child)
			{
				...
				try {
					...
					TVP_CHECK_MANAGER("Draw:child->Draw(cache)");
				} catch(...) {
					TVPAddLog(TJS_W("[DRAW] child->Draw() threw, skipping"));
				}
			}
			TVP_LAYER_FOR_EACH_CHILD_END
		...
		TVP_CHECK_MANAGER("Draw:DrawCompleted");
	}
	else
	{
		...
		if(GetVisibleChildrenCount() == 0)
		{
			...
			TVP_CHECK_MANAGER("Draw:DrawSelf");
		}
		else
		{
			...
			while(it.Step())
			{
				...
				if(!UpdateBitmapForChild) continue;
				...
				try
				{
					...
					TVP_LAYER_FOR_EACH_CHILD_BEGIN(child)
					{
						...
						TVP_CHECK_MANAGER("Draw:child->Draw(overlapped)");
					}
					TVP_LAYER_FOR_EACH_CHILD_END
			...
			while(it.Step())
			{
				...
				else
				{
					...
					TVP_LAYER_FOR_EACH_CHILD_BEGIN(child)
					{
						...
						// call children's "Draw" method
						child->Draw((tTVPDrawable*)this, chrect, true);
						TVP_CHECK_MANAGER("Draw:child->Draw(exposed)");
					}
					TVP_LAYER_FOR_EACH_CHILD_END
	...
	tTVPRect &cliprect)
{
	...
	{
		if(!CurrentDrawTarget) return NULL;
		tTVPRect _rect(rect);
	...
	tTVPLayerType type, tjs_int opacity)
{
	...
	{
		if(!UpdateBitmapForChild) return;
		if(MainImage == NULL)
		{
...
{
	void* _tvp_mgr_saved = Manager;
	if(tvpIsValidWasmPtr(Manager)) Manager->SetRendering(true);

	BeforeCompletion();
	TVP_CHECK_MANAGER("IC:BeforeCompletion");
	...
	TVP_CHECK_MANAGER("IC:InternalComplete2");

	InCompletion = false;
	if(tvpIsValidWasmPtr(Manager)) Manager->SetRendering(false);
	AfterCompletion();
	if(tvpIsValidWasmPtr(Manager)) Manager->DrainDeferredInvalidates();
}
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::CompleteForWindow(tTVPDrawable *drawable)
{
	if(!tvpIsValidWasmPtr(Manager)) return;
	void* _tvp_mgr_saved = Manager;
	// Sanity check: Manager should be 4-byte aligned and within reasonable heap range
	uintptr_t maddr = (uintptr_t)Manager;
	if((maddr & 3) != 0 || maddr < 0x10000) {
		fprintf(stderr, "[CFW] Manager corrupted: %p, skipping render\n", (void*)Manager);
		fflush(stderr);
		return;
	}
	...
	maddr = (uintptr_t)Manager;
	if((maddr & 3) != 0 || maddr < 0x10000) {
		fprintf(stderr, "[CFW] Manager corrupted after BeforeCompletion: %p\n", (void*)Manager);
		fflush(stderr);
		return;
	}
	...
	auto owner = Manager->GetLayerTreeOwner();
	if(!owner || !tvpIsValidWasmPtr(owner)) {
		InCompletion = false;
		if(tvpIsValidWasmPtr(Manager)) Manager->SetRendering(false);
		return;
	}
	owner->StartBitmapCompletion(Manager);
	try
	{
		InternalComplete2(Manager->GetUpdateRegionForCompletion(), drawable);
		TVP_CHECK_MANAGER("CFW:InternalComplete2");
	}
	catch(...)
	{
		if(tvpIsValidWasmPtr(Manager)) Manager->GetLayerTreeOwner()->EndBitmapCompletion(Manager);
		InCompletion = false;
		if(tvpIsValidWasmPtr(Manager)) { Manager->SetRendering(false); Manager->DrainDeferredInvalidates(); }
		throw;
	}
	if(tvpIsValidWasmPtr(Manager)) Manager->GetLayerTreeOwner()->EndBitmapCompletion(Manager);
...
{
#ifdef __EMSCRIPTEN__
	KRKR_LOG_L3("[DIAG-TRANS] StartTransition called: name=%s withchildren=%d transsource=%p\n",
		name.AsNarrowStdString().c_str(), (int)withchildren, (void*)transsource);
#endif
	// start transition
...
void tTJSNI_BaseLayer::InternalStopTransition()
{
	...
	{
#ifdef __EMSCRIPTEN__
		KRKR_LOG_L3("[DIAG-TRANS] InternalStopTransition: InTransition=true, will fire onTransitionCompleted\n");
#endif
		InTransition = false;
		TransCompEventPrevented = false;
	...
	tjs_int dx, tjs_int dy, const tTVPRect &srcrect)
{
	...
	if(data.Dest == data.Src1)
	{
		...
		if(dest && MainImage) dest->CopyRect(dx, dy, MainImage, srcrect);
	}
	else if(data.Dest == data.Src2)
	{
		// (returned destination is data.Src2)
		if(dest && TransSrc && TransSrc->MainImage) dest->CopyRect(dx, dy, TransSrc->MainImage, srcrect);
	}
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/clear)
{
	...
	if(numparams >= 2 && param[1]->Type() != tvtVoid)
	{
		tTVPDrawFace oldface = _this->GetFace();
		_this->SetFace((tTVPDrawFace)(tjs_int)*param[0]);
		color = static_cast<tjs_uint32>((tjs_int64)*param[1]);
		_this->FillRect(tTVPRect(0, 0, _this->GetImageWidth(), _this->GetImageHeight()), color);
		_this->SetFace(oldface);
	}
	else
	{
		if(numparams >= 1 && param[0]->Type() != tvtVoid)
...
// E-mote plugin (emotefilter.dll) stub properties.
// KAG's animkaglayer.tjs accesses these on every frame via the continuous
// handler. Without them, the member-not-found exception kills the conductor.
// [VALIDATION-DISABLED] These property stubs were tricking the game's
// motionplayer-fallback detection. Kirikiroid2 upstream does not have them.
#if 0
TJS_BEGIN_NATIVE_PROP_DECL(allplaying)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		*result = (tjs_int)0;
...
TJS_END_NATIVE_PROP_DECL(allplaying)
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(motionPlaying)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		*result = (tjs_int)0;
...
TJS_END_NATIVE_PROP_DECL(motionPlaying)
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(emoteAvail)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		*result = (tjs_int)0;
...
TJS_END_NATIVE_PROP_DECL(emoteAvail)
#endif
//----------------------------------------------------------------------
// animkaglayer.tjs stores _motionPlaying as a script-side private member,
// but stale continuous handlers can occasionally run with a Layer context
// during menu teardown. Provide a narrow default so the private-state read
// does not become a fatal member-not-found exception.
TJS_BEGIN_NATIVE_PROP_DECL(_motionPlaying)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		*result = false;
...
TJS_END_NATIVE_PROP_DECL(_motionPlaying)
//----------------------------------------------------------------------
// animkaglayer.tjs calls layer.contains(x, y) in onHitTest.
// Without this, every mouse event throws "Member contains does not exist"
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/contains)
{
	...
	tjs_int y = *param[1];
	if(result)
		*result = (tjs_int)(x >= 0 && y >= 0 &&
			x < (tjs_int)_this->GetWidth() &&
			y < (tjs_int)_this->GetHeight());
	return TJS_S_OK;
}
TJS_END_NATIVE_METHOD_DECL(/*func. name*/contains)
//----------------------------------------------------------------------
// affinelayer.tjs(525) onPaint calls layer.clipAlphaRect(dx, dy, src,
// sx, sy, w, h, [clearAlpha]). It multiplies destination alpha by source
// alpha over the clipped rectangle: dstA = dstA * srcA / 255. When the
// optional clearAlpha is supplied, every alpha byte outside the clipped
// destination region is filled with that value. Ported from krkrsdl3
// LayerExBTOA.cpp clipAlphaRect (https://github.com/luxiaoling-mc/krkrsdl3).
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/clipAlphaRect)
{
	TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Layer);
	if(numparams < 7) return TJS_E_BADPARAMCOUNT;

	tjs_int dx = (tjs_int)*param[0];
	tjs_int dy = (tjs_int)*param[1];
	...
	tjs_int sx = (tjs_int)*param[3];
	tjs_int sy = (tjs_int)*param[4];
	tjs_int w  = (tjs_int)*param[5];
	tjs_int h  = (tjs_int)*param[6];

	bool clr = false;
	unsigned char clrval = 0;
	if(numparams >= 8 && param[7]->Type() != tvtVoid)
	{
		tjs_int n = (tjs_int)*param[7];
		clr = (n >= 0 && n < 256);
		clrval = (unsigned char)(n & 0xff);
	}
	if(w <= 0 || h <= 0) return TJS_E_INVALIDPARAM;

	tjs_int diw = (tjs_int)_this->GetImageWidth();
	tjs_int dih = (tjs_int)_this->GetImageHeight();
	tjs_int dpitch = _this->GetMainImagePixelBufferPitch();
	tjs_int siw = (tjs_int)src->GetImageWidth();
	tjs_int sih = (tjs_int)src->GetImageHeight();
	tjs_int spitch = src->GetMainImagePixelBufferPitch();

	unsigned char *dbuf = (unsigned char *)_this->GetMainImagePixelBufferForWrite();
	const unsigned char *sbuf = (const unsigned char *)src->GetMainImagePixelBuffer();
	if(!dbuf) TVPThrowExceptionMessage(TJS_W("dest has no image"));
	if(!sbuf) TVPThrowExceptionMessage(TJS_W("src has no image"));
	...
	if(!inRange)
	{
		if(clr)
		{
			for(tjs_int y = 0; y < dih; y++)
			{
				unsigned char *p = dbuf + y * dpitch + 3;
				for(tjs_int x = 0; x < diw; x++, p += 4) *p = clrval;
			}
		}
		return TJS_S_OK;
	}
	...
	if(clr)
	{
		for(tjs_int y = 0; y < dy; y++)
		{
			unsigned char *p = dbuf + y * dpitch + 3;
			for(tjs_int x = 0; x < diw; x++, p += 4) *p = clrval;
		}
		for(tjs_int y = dy + h; y < dih; y++)
		{
			unsigned char *p = dbuf + y * dpitch + 3;
			for(tjs_int x = 0; x < diw; x++, p += 4) *p = clrval;
		}
	...
	for(tjs_int y = 0; y < h; y++)
	{
		...
		if(clr)
		{
			p = dbuf + (y + dy) * dpitch + 3;
			for(tjs_int x = 0; x < dx; x++, p += 4) *p = clrval;
		}
		p = dbuf + (y + dy) * dpitch + 3 + dx * 4;
		const unsigned char *q = sbuf + (y + sy) * spitch + 3 + sx * 4;
		for(tjs_int x = 0; x < w; x++, p += 4, q += 4)
		{
		...
		if(clr)
		{
			p = dbuf + (y + dy) * dpitch + 3 + (dx + w) * 4;
			for(tjs_int x = dx + w; x < diw; x++, p += 4) *p = clrval;
		}
	}
	return TJS_S_OK;
}
TJS_END_NATIVE_METHOD_DECL(/*func. name*/clipAlphaRect)
#endif
//----------------------------------------------------------------------
	...
	{
		tTJSVariant val;
		tjs_error hr = PropGet(TJS_MEMBERMUSTEXIST, TJS_W("contains"), NULL, &val, this);
		KRKR_LOG_L2("[DIAG-C] contains on Layer cls: hr=%d\n", (int)hr);
		hr = PropGet(TJS_MEMBERMUSTEXIST, TJS_W("_motionPlaying"), NULL, &val, this);
		KRKR_LOG_L2("[DIAG-C] _motionPlaying on Layer cls: hr=%d\n", (int)hr);
		hr = PropGet(TJS_MEMBERMUSTEXIST, TJS_W("emoteAvail"), NULL, &val, this);
		KRKR_LOG_L2("[DIAG-C] emoteAvail on Layer cls: hr=%d\n", (int)hr);
	}
#endif
