...
{
	if (!IsBmpValid()) return NULL;
#ifdef __EMSCRIPTEN__
	uintptr_t heapSize = (uintptr_t)(__builtin_wasm_memory_size(0)) * 65536;
	if (!BitmapInfo || (uintptr_t)BitmapInfo >= heapSize) return NULL;
	if (!Bits || (uintptr_t)Bits >= heapSize) return NULL;
#else
	if (!BitmapInfo || !Bits) return NULL;
#endif
	if (BitmapInfo->GetHeight() == 0) return NULL;
	if((tjs_int)l>=BitmapInfo->GetHeight()) return NULL;
	if(PitchBytes <= 0 || PitchBytes > 65536) return NULL;
...
tjs_uint tTVPNativeBaseBitmap::GetWidth() const
{
	if (!Bitmap) return 0;
#ifdef __EMSCRIPTEN__
	if ((uintptr_t)Bitmap > (uintptr_t)(__builtin_wasm_memory_size(0) * 65536))
		return 0;
#endif
...
const void * tTVPNativeBaseBitmap::GetScanLine(tjs_uint l) const
{
	...
	if ((uintptr_t)Bitmap > (uintptr_t)(__builtin_wasm_memory_size(0) * 65536))
		return NULL;
#endif
	if (!Bitmap->IsBmpValid()) return NULL;
...
void * tTVPNativeBaseBitmap::GetScanLineForWrite(tjs_uint l)
{
	...
	if (!Bitmap->IsBmpValid()) return NULL;
	Independ();
	if (!Bitmap || !Bitmap->IsBmpValid()) return NULL;
	return Bitmap->GetScanLine(l);
}
//---------------------------------------------------------------------------
...
{
	tTVPBitmap *newb = new tTVPBitmap(w, h, bpp, unpadding);
