...
class tTVPLayerManager : public iTVPLayerManager, public tTVPDrawable
{
	...
	// WASM: deferred destruction during rendering
	bool Rendering;
	struct tDeferredInvalidateItem
	{
		tTJSNI_BaseLayer *Layer;
		iTJSDispatch2 *Owner;
	};
	std::vector<tDeferredInvalidateItem> DeferredInvalidateQueue;

public:
	bool IsInRendering() const { return Rendering; }
	void SetRendering(bool v) { Rendering = v; }
	void DeferInvalidate(tTJSNI_BaseLayer* layer);
	void DrainDeferredInvalidates();
