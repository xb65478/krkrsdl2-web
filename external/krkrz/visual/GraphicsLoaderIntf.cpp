	...
	tTVPGraphicLoadMode mode, ttstr *provincename, iTJSDispatch2 ** metainfo, bool unpadding)
{
	...
	std::string perfRawName = name.AsNarrowStdString();
	bool perfTracePsb = TVPPerfEnabled() && (perfRawName.find(".psb") != std::string::npos ||
		perfRawName.find(".PSB") != std::string::npos);
	tjs_uint64 perfStart = perfTracePsb ? TVPGetTickCount() : 0;
	if (perfTracePsb) {
		KRKR_LOG_L2("[GFX-LOAD] TVPLoadGraphic PSB file: %s\n", perfRawName.c_str());
	}
#endif
	// loading with cache management
	ttstr nname = TVPNormalizeStorageName(name);
#ifdef __EMSCRIPTEN__
	tjs_uint64 perfAfterNormalize = perfTracePsb ? TVPGetTickCount() : 0;
	std::string perfNormalizedName = perfTracePsb ? nname.AsNarrowStdString() : std::string();
	tjs_uint64 perfCacheLookupStart = perfTracePsb ? TVPGetTickCount() : 0;
	tjs_uint64 perfAfterCacheLookup = perfCacheLookupStart;
#endif
	tjs_uint32 hash;
	tTVPGraphicsSearchData searchdata;
...
#ifdef __EMSCRIPTEN__
		perfCacheLookupStart = perfTracePsb ? TVPGetTickCount() : perfCacheLookupStart;
#endif
		tTVPGraphicImageHolder * ptr =
			TVPGraphicCache.FindAndTouchWithHash(searchdata, hash);
#ifdef __EMSCRIPTEN__
		perfAfterCacheLookup = perfTracePsb ? TVPGetTickCount() : perfAfterCacheLookup;
#endif
		if(ptr)
		{
			// found in cache
#ifdef __EMSCRIPTEN__
			tjs_uint64 perfAssignStart = perfTracePsb ? TVPGetTickCount() : 0;
#endif
			ptr->GetObjectNoAddRef()->AssignToBitmap(dest);
			...
			if (perfTracePsb) {
				...
				fprintf(stderr,
					"[PERF] gfx.load path=%s normalized=%s cache_enabled=1 cache=hit keyidx=%d mode=%d des=%ux%u normalize_ms=%llu cache_lookup_ms=%llu assign_ms=%llu total_ms=%llu\n",
					perfRawName.c_str(),
					perfNormalizedName.c_str(),
					(int)keyidx,
					(int)mode,
					(unsigned)desw,
					(unsigned)desh,
					(unsigned long long)(perfAfterNormalize - perfStart),
					(unsigned long long)(perfAfterCacheLookup - perfCacheLookupStart),
					(unsigned long long)(perfEnd - perfAssignStart),
...
#ifdef __EMSCRIPTEN__
	tjs_uint64 perfInternalStart = perfTracePsb ? TVPGetTickCount() : 0;
	tjs_uint64 perfAfterInternal = perfInternalStart;
	tjs_uint64 perfCacheStoreStart = perfInternalStart;
	tjs_uint64 perfAfterCacheStore = perfInternalStart;
#endif
	try
	{
		TVPInternalLoadGraphic(dest, nname, keyidx, desw, desh, &mi, mode, &pn, unpadding);
#ifdef __EMSCRIPTEN__
		perfAfterInternal = perfTracePsb ? TVPGetTickCount() : perfAfterInternal;
#endif
		...
		{
#ifdef __EMSCRIPTEN__
			perfCacheStoreStart = perfTracePsb ? TVPGetTickCount() : perfCacheStoreStart;
#endif
			data = new tTVPGraphicImageData();
...
//			}
#ifdef __EMSCRIPTEN__
			perfAfterCacheStore = perfTracePsb ? TVPGetTickCount() : perfAfterCacheStore;
#endif
		}
#ifdef __EMSCRIPTEN__
		if (perfTracePsb) {
			tjs_uint64 perfEnd = TVPGetTickCount();
			fprintf(stderr,
				"[PERF] gfx.load path=%s normalized=%s cache_enabled=%d cache=miss keyidx=%d mode=%d des=%ux%u normalize_ms=%llu cache_lookup_ms=%llu internal_ms=%llu cache_store_ms=%llu total_ms=%llu\n",
				perfRawName.c_str(),
				perfNormalizedName.c_str(),
				TVPGraphicCacheEnabled ? 1 : 0,
				(int)keyidx,
				(int)mode,
				(unsigned)desw,
				(unsigned)desh,
				(unsigned long long)(perfAfterNormalize - perfStart),
				(unsigned long long)(perfAfterCacheLookup - perfCacheLookupStart),
				(unsigned long long)(perfAfterInternal - perfInternalStart),
				(unsigned long long)(perfAfterCacheStore - perfCacheStoreStart),
				(unsigned long long)(perfEnd - perfStart));
		}
#endif
	}
	catch(...)
	{
		...
		if (perfTracePsb) {
			tjs_uint64 perfEnd = TVPGetTickCount();
			fprintf(stderr,
				"[PERF] gfx.load path=%s normalized=%s cache_enabled=%d cache=miss failed=1 keyidx=%d mode=%d des=%ux%u normalize_ms=%llu cache_lookup_ms=%llu internal_ms=%llu total_ms=%llu\n",
				perfRawName.c_str(),
				perfNormalizedName.c_str(),
				TVPGraphicCacheEnabled ? 1 : 0,
				(int)keyidx,
				...
				(unsigned long long)(perfAfterNormalize - perfStart),
				(unsigned long long)(perfAfterCacheLookup - perfCacheLookupStart),
				(unsigned long long)(perfEnd - perfInternalStart),
				(unsigned long long)(perfEnd - perfStart));
		}
#endif
		if(mi) delete mi;
