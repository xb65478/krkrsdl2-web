...
void TJS_INTF_METHOD tTVPDrawDevice::OnClick(tjs_int x, tjs_int y)
{
...
#ifdef __EMSCRIPTEN__
	{
		...
		log_count++;
		if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-DRAW] onClick window=(%d,%d) primary=(%d,%d)\n", (int)ox, (int)oy, (int)x, (int)y);
	}
#endif
	iTVPLayerManager * manager = GetLayerManagerAt(PrimaryLayerManagerIndex);
	if(!manager) return;
...
void TJS_INTF_METHOD tTVPDrawDevice::OnMouseDown(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
		if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-DRAW] onMouseDown window=(%d,%d) primary=(%d,%d) mb=%d flags=%u\n", (int)ox, (int)oy, (int)x, (int)y, (int)mb, (unsigned)flags);
	}
#endif
	iTVPLayerManager * manager = GetLayerManagerAt(PrimaryLayerManagerIndex);
	if(!manager) return;
...
void TJS_INTF_METHOD tTVPDrawDevice::OnMouseUp(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags)
{
...
#ifdef __EMSCRIPTEN__
	{
		static int log_count = 0;
		log_count++;
		if (TVPLogL3() && log_count <= 80) fprintf(stderr, "[INPUT-DRAW] onMouseUp window=(%d,%d) primary=(%d,%d) mb=%d flags=%u\n", (int)ox, (int)oy, (int)x, (int)y, (int)mb, (unsigned)flags);
	}
#endif
	iTVPLayerManager * manager = GetLayerManagerAt(PrimaryLayerManagerIndex);
	if(!manager) return;
