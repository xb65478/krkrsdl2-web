...
static bool TVPDispatchMotionEventBridge(iTJSDispatch2* tag,
    const ttstr& sourceStorage, const ttstr& sourceLabel) {
    if (!tag)
        return false;
    ...
    tjs_error er = global->PropGet(TJS_MEMBERMUSTEXIST,
        TJS_W("__krkrMotionEventBridge"), NULL, &bridgeVar, global);
    ...
    bool ok = false;
    try {
        tjs_error hr = bridgeVar.AsObjectClosureNoAddRef().FuncCall(0, NULL,
            NULL, &result, 1, params, NULL);
...
static bool TVPIsMotionEventBridgeWaiting() {
    ...
    tTJSVariant fnVar;
    tjs_error er = global->PropGet(TJS_MEMBERMUSTEXIST,
        TJS_W("__krkrMotionEventShouldWait"), NULL, &fnVar, global);
    ...
    bool waiting = false;
    try {
        tjs_error hr = fnVar.AsObjectClosureNoAddRef().FuncCall(0, NULL, NULL,
            &result, 0, NULL, NULL);
...
//---------------------------------------------------------------------------
void tTVPScenarioCacheItemEX::AddRef() { RefCount++; }
//---------------------------------------------------------------------------
            ...
            if (Lines[i].Length >= 2 && Lines[i].Start[0] == TJS_W('*')) {
                ...
                if (!label.c_str()[1]) {
                    if (prevlabel.IsEmpty())
                        TVPThrowExceptionMessage(TVP_KAGPARSER_MESSAGEMAP(
                            TVPKAGEXCannotOmmitFirstLabelName));
                    label = prevlabel;
                }
...
typedef TJSEX::tTJSHashCache<ttstr, tTVPScenarioCacheItemHolder, TJSEX::tTJSHashFunc<ttstr>,
    (TVP_SCENARIO_MAX_CACHE_SIZE * 2)>
    tTVPScenarioCache;
static tTVPScenarioCache TVPScenarioCache(TVP_SCENARIO_MAX_CACHE_SIZE);
//---------------------------------------------------------------------------
void TVPClearScnearioCacheEX() { TVPScenarioCache.Clear(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
static tTVPScenarioCacheItemEX* TVPGetScenario(const ttstr& storagename,
    bool isstring) {
...
//---------------------------------------------------------------------------
tjs_error tTJSNI_KAGParserEX::Construct(tjs_int numparams, tTJSVariant** param,
...
void tTJSNI_KAGParserEX::operator=(const tTJSNI_KAGParserEX& ref) {
    ...
    if (Scenario != ref.Scenario) {
        if (Scenario)
            Scenario->Release(), Scenario = NULL, Lines = NULL,
            CurLineStr = NULL;
...
void tTJSNI_KAGParserEX::RestoreIntStackFromStr(std::vector<tjs_int>& stack,
    const ttstr& str) {
    stack.clear();
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParserEX::RestoreBoolStackFromStr(std::vector<bool>& stack,
    const ttstr& str) {
    stack.clear();
...

    {
        ...
        if (MacroArgStackDepth != MacroArgs.size())
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXMalformedSaveData));
...
void tTJSNI_KAGParserEX::GoToLabel(const ttstr& name) {
    ...
    if (newline) {
        ...
        // not found
        TVPThrowExceptionMessage(TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXLabelNotFound),
            StorageName, name);
    }
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParserEX::GoToStorageAndLabel(const ttstr& storage,
    const ttstr& label) {
    if (!storage.IsEmpty())
        LoadScenario(storage);
    if (!label.IsEmpty())
...
bool tTJSNI_KAGParserEX::SkipCommentOrLabel() {
    ...
    for (; CurLine < LineCount; CurLine++) {
        ...
        if (p[0] == TJS_W('[') &&
            ...
            if (CurLine == LineCount)
                TVPThrowExceptionMessage(
                    TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXInlineScriptNotEnd));
...
tTJSNI_KAGParserEX::ArgValue&
tTJSNI_KAGParserEX::ArgValue::operator=(const ArgValue& right) {
...
void tTJSNI_KAGParserEX::ArgValue::add(tTJSVariant& name, tTJSVariant& value) {
    ttstr strName = name;
    dic->PropSetByVS(TJS_MEMBERENSURE, strName.AsVariantStringNoAddRef(),
        &value, dic);
    tTJSVariant* args[1];
    args[0] = &name;
    ArrayPush->FuncCall(0, NULL, NULL, NULL, 1, args, array);
...
tjs_error tTJSNI_KAGParserEX::ArgValue::getProp(ttstr& name,
    tTJSVariant& value) const {
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParserEX::FindNearestLabel(tjs_int start, tjs_int& labelline,
    ttstr& labelname) {
...
void tTJSNI_KAGParserEX::PopCallStack(const ttstr& storage, const ttstr& label) {
    ...
    if (CallStack.size() == 0)
        TVPThrowExceptionMessage(
            TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXCallStackUnderflow));
    ...
    if (!storage.IsEmpty() || !label.IsEmpty()) {
        ...
        if (CurLine > LineCount)
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXReturnLostSync));
...
bool tTJSNI_KAGParserEX::EntryParam(bool& condition, tTJSVariant& ValueVariant,
    const ttstr& attribname, const ttstr& value,
    bool entity = false, bool macroarg = false) {
    // extract parameter macro
    tTJSVariant paramMacroValue;
    if (TJS_SUCCEEDED(ParamMacros->PropGet(0, attribname.c_str(), NULL,
...
#ifdef __EMSCRIPTEN__
iTJSDispatch2* tTJSNI_KAGParserEX::CreateMotionEventWaitTag() {
    args.clear();
    ...
    args.add(tag_name, tag_val);
    args.add(time_name, time_val);
    return args.getReturn();
}

bool tTJSNI_KAGParserEX::HandleMotionEventBridgeTag(iTJSDispatch2* tag) {
    if (!tag)
        return false;

    tTJSVariant tn_var;
    if (TJS_FAILED(tag->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("tagname"), NULL,
        &tn_var, tag)) || tn_var.Type() != tvtString) {
        return false;
    }
    ...
    if (TJS_SUCCEEDED(tag->PropGet(0, TJS_W("storage"), NULL, &sv, tag)) &&
        sv.Type() != tvtVoid) {
        storage = ttstr(sv);
        std::string sn = storage.AsLowerCase().AsNarrowStdString();
        dispatch = sn.find(".psb") != std::string::npos;
    }
    if (TJS_SUCCEEDED(tag->PropGet(0, TJS_W("chara"), NULL, &cv, tag)) &&
        cv.Type() != tvtVoid) {
        chara = ttstr(cv);
    }
    if (TJS_SUCCEEDED(tag->PropGet(0, TJS_W("motion"), NULL, &mv, tag)) &&
        mv.Type() != tvtVoid) {
        motion = ttstr(mv);
    }
    if (TJS_SUCCEEDED(tag->PropGet(0, TJS_W("waitmovie"), NULL, &wmv, tag)) &&
        wmv.Type() != tvtVoid) {
        waitmovie = ttstr(wmv);
        dispatch = true;
    }
    if (TJS_SUCCEEDED(tag->PropGet(0, TJS_W("notrans"), NULL, &ntv, tag)) &&
        ntv.Type() != tvtVoid) {
        notrans = ttstr(ntv);
    }
    ...
    if (TVPIsDracuBringupEnabled()) {
        TVPLogDracuKagBridge("enabled",
            storage.GetLen() ? "ev-psb" : "ev-waitmovie");
    }
    bool ok = TVPDispatchMotionEventBridge(tag, StorageName, CurLabel);
    if (waitmovie.GetLen() && ok && TVPIsMotionEventBridgeWaiting()) {
        MotionEventWaitActive = true;
        MotionEventWaitSyntheticCount = 0;
        KRKR_LOG_L2("[KAG-MOTION-EV] waitmovie armed ks=%s lbl=%s\n",
            StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
            CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
    }
    KRKR_LOG_L2("[KAG-MOTION-EV] dispatched=%d storage=%s chara=%s motion=%s waitmovie=%s notrans=%s ks=%s lbl=%s\n",
        ok ? 1 : 0,
        storage.GetLen() ? TVPDiagUtf8(storage).c_str() : "-",
        chara.GetLen() ? TVPDiagUtf8(chara).c_str() : "-",
        motion.GetLen() ? TVPDiagUtf8(motion).c_str() : "-",
        waitmovie.GetLen() ? TVPDiagUtf8(waitmovie).c_str() : "-",
        notrans.GetLen() ? TVPDiagUtf8(notrans).c_str() : "-",
        StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
        CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
    return ok && storage.GetLen() > 0;
}
#endif
...
iTJSDispatch2* tTJSNI_KAGParserEX::GetNextTag() {
    ...
    if (MotionEventWaitActive) {
        if (TVPIsMotionEventBridgeWaiting()) {
            ...
            if (MotionEventWaitSyntheticCount <= 8 ||
                (MotionEventWaitSyntheticCount % 30) == 0) {
                KRKR_LOG_L2("[KAG-MOTION-EV] waitmovie blocking count=%d ks=%s lbl=%s\n",
        ...
        KRKR_LOG_L2("[KAG-MOTION-EV] waitmovie released count=%d ks=%s lbl=%s\n",
            (int)MotionEventWaitSyntheticCount,
            StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
            CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
        MotionEventWaitActive = false;
...
parse_start:
    ...
    while (true) {
        ...
        if (Interrupted) {
            ...
            Interrupted = false;
            iTJSDispatch2* result = args.getReturn();
#ifdef __EMSCRIPTEN__
            if (HandleMotionEventBridgeTag(result)) {
                result->Release();
                return CreateMotionEventWaitTag();
            }
#endif
...
#define TVP_KAG_STEP_NEXT                                                      \
    if(multiLine > CurLine) {                                                  \
        ...
        while (true) {
            ...
            if (CurLineStr[CurPos] == ldelim) {
                ...
                if (condition && ExcludeLevel == -1) {
                    ...
                    ismacro = TJS_SUCCEEDED(Macros->PropGet(
                        0, tagname.c_str(), NULL, &macroval, Macros));
                    if (ismacro)
                    ...
                    if (tagkind == tag_emb ||
                        ...
                        if (tagkind == tag_macro) {
                            ...
                            struct ExtractParamMacro {
                                ...
                                void operator()(tTJSVariant& name,
                                    tTJSVariant& value) {
                                    ttstr n = name;
                                    if (n != __name_name && n != __tag_name) {
                                        tTJSVariant* args[] = { &name, &value };
                                        ArrayPush->FuncCall(0, NULL, NULL, NULL,
                                            2, args, array);
                                    }
                                }
                                iTJSDispatch2* array;
            ...
            if ((!RecordingMacro && ExcludeLevel == -1) ||
                tagkind == tag_elsif) {
                store = EntryParam(condition, ValueVariant, attribname, value,
                    entity, macroarg);
            }
    ...
    TJS_END_NATIVE_METHOD_DECL(/*func. name*/ getNextTag)
        ...
        if (clo.Object) {
            if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                TJS_NIS_GETINSTANCE, tTJSNC_KAGParser::ClassID,
                (iTJSNativeInstance**)&src)))
                TVPThrowExceptionMessage(
                    TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXSpecifyKAGParser));
        }
        else {
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGEXSpecifyKAGParser));
...
// krkrsdl2 static-integration helpers: called from SysInitImpl / PluginImpl.
// Normally the ncbind PRE_REGIST_CALLBACK in KAG.cpp does this at DLL load, but
// we statically link, so KAG.cpp is not compiled and we expose C-linkage entry points.
// The guard prevents double-init if both paths fire (e.g. startup + Plugins.link).
static bool s_KAGParserEX_initialized = false;

extern "C" void TVPKAGParserEX_StaticInit() {
    if (s_KAGParserEX_initialized) return;
    tTJSNI_KAGParserEX::initMethod();
    s_KAGParserEX_initialized = true;
}

extern "C" void TVPKAGParserEX_StaticDone() {
    if (!s_KAGParserEX_initialized) return;
    tTJSNI_KAGParserEX::doneMethod();
    s_KAGParserEX_initialized = false;
}
