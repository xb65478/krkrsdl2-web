...
#ifdef __EMSCRIPTEN__
static std::string TVPDiagUtf8(const ttstr &s)
{
	tjs_int len = TVPWideCharToUtf8String(s.c_str(), NULL);
	if(len < 0) return s.AsNarrowStdString();
	std::string out((size_t)len, '\0');
	if(len > 0) TVPWideCharToUtf8String(s.c_str(), &out[0]);
	return out;
}
...
static void TVPLogDracuKagBridge(const char *state, const char *tag)
{
	static int log_count = 0;
	if (log_count++ >= 16) return;
	KRKR_LOG_L2(
		"[HACK-P0] id=P0-004 state=%s feature=kag-emote-bridge tag=%s target=DRACU flag=-krkr-dracu-bringup\n",
		state, tag ? tag : "-");
}

static bool TVPDispatchMotionEventBridge(iTJSDispatch2 *tag, const ttstr &sourceStorage, const ttstr &sourceLabel)
{
	if (!tag) return false;
	iTJSDispatch2 *global = TVPGetScriptDispatch();
	if (!global) return false;

	tTJSVariant bridgeVar;
	tjs_error er = global->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("__krkrMotionEventBridge"), nullptr, &bridgeVar, global);
	if (TJS_FAILED(er) || bridgeVar.Type() == tvtVoid) {
		fprintf(stderr, "[KAG-MOTION-EV] bridge missing ks=%s lbl=%s\n",
			sourceStorage.GetLen() ? TVPDiagUtf8(sourceStorage).c_str() : "-",
			sourceLabel.GetLen() ? TVPDiagUtf8(sourceLabel).c_str() : "-");
		fflush(stderr);
		global->Release();
		return false;
	}
	...
	tTJSVariant result;
	tTJSVariant tagVar(tag, tag);
	tTJSVariant *params[] = { &tagVar };
	bool ok = false;
	try {
		tjs_error hr = bridgeVar.AsObjectClosureNoAddRef().FuncCall(0, nullptr, nullptr, &result, 1, params, nullptr);
		ok = TJS_SUCCEEDED(hr) && result.operator bool();
	}
	catch(eTJS &e) {
		fprintf(stderr, "[KAG-MOTION-EV] dispatch threw ks=%s lbl=%s error=%s\n",
			sourceStorage.GetLen() ? TVPDiagUtf8(sourceStorage).c_str() : "-",
			sourceLabel.GetLen() ? TVPDiagUtf8(sourceLabel).c_str() : "-",
		...
		fprintf(stderr, "[KAG-MOTION-EV] dispatch threw ks=%s lbl=%s error=unknown\n",
			sourceStorage.GetLen() ? TVPDiagUtf8(sourceStorage).c_str() : "-",
			sourceLabel.GetLen() ? TVPDiagUtf8(sourceLabel).c_str() : "-");
		fflush(stderr);
		global->Release();
...
static bool TVPIsMotionEventBridgeWaiting()
{
	iTJSDispatch2 *global = TVPGetScriptDispatch();
	if (!global) return false;

	tTJSVariant fnVar;
	tjs_error er = global->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("__krkrMotionEventShouldWait"), nullptr, &fnVar, global);
	if (TJS_FAILED(er) || fnVar.Type() == tvtVoid) {
		global->Release();
		return false;
	}
	...
	bool waiting = false;
	try {
		tjs_error hr = fnVar.AsObjectClosureNoAddRef().FuncCall(0, nullptr, nullptr, &result, 0, nullptr, nullptr);
		waiting = TJS_SUCCEEDED(hr) && result.operator bool();
	}
	catch(eTJS &e) {
		fprintf(stderr, "[KAG-MOTION-EV] shouldWait threw error=%s\n",
			e.GetMessage().AsNarrowStdString().c_str());
		fflush(stderr);
		waiting = false;
	}
	catch(...) {
		fprintf(stderr, "[KAG-MOTION-EV] shouldWait threw error=unknown\n");
		fflush(stderr);
		waiting = false;
	}
...
//			stream = TVPCreateTextStreamForReadByEncoding(name, TJS_W(""), TJS_W("Shift_JIS"));
			...
			Buffer = tmp.c_str();
#ifdef __EMSCRIPTEN__
			// [PLAN-B+] Dump decrypted scenario content for key fallback files
			std::string nm = name.AsNarrowStdString();
			bool dump = false;
			for (auto kw : {"compat", "macro", "first", "custom", "motion", "title.ks", "logo", "start.ks", "click_wait", ".asd", "プロローグ", "★"}) {
				if (nm.find(kw) != std::string::npos) { dump = true; break; }
			}
			if (dump) {
				std::string content = TVPDiagUtf8(ttstr(Buffer));
				// Replace embedded NULs so chunked fprintf("%s") doesn't truncate
				// (krkr ttstr is wide-char; AsNarrowStdString yields UTF-8, but a
				// few rare scripts still have \0 bytes from ttstr quirks).
				for (auto &c : content) if (c == '\0') c = ' ';
				size_t plen = content.size();
				fprintf(stderr, "[KS-DUMP] file=%s len=%zu BEGIN\n", nm.c_str(), plen);
				size_t i = 0;
				while (i < plen) {
					size_t end = i + 800;
					if (end > plen) end = plen;
					fprintf(stderr, "[KS-DUMP] %s [%zu..%zu] >>>", nm.c_str(), i, end);
					fwrite(content.data() + i, 1, end - i, stderr);
					fprintf(stderr, "<<<\n");
					i = end;
				}
				fprintf(stderr, "[KS-DUMP] file=%s END\n", nm.c_str());
				fflush(stderr);
			}
#endif
		}
...
tTJSNI_KAGParser::tTJSNI_KAGParser()
{
	...
	Interrupted = false;
#ifdef __EMSCRIPTEN__
	MotionEventWaitActive = false;
	MotionEventWaitSyntheticCount = 0;
#endif
...
#ifdef __EMSCRIPTEN__
	MotionEventWaitActive = ref.MotionEventWaitActive;
	MotionEventWaitSyntheticCount = ref.MotionEventWaitSyntheticCount;
#endif
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
iTJSDispatch2 * tTJSNI_KAGParser::CreateMotionEventWaitTag()
{
	DicClear->FuncCall(0, NULL, NULL, NULL, 0, NULL, DicObj);
	static ttstr tag_name(TJSMapGlobalStringMap(TJS_W("tagname")));
	static ttstr time_name(TJSMapGlobalStringMap(TJS_W("time")));
	tTJSVariant tag_val(TJS_W("wait"));
	tTJSVariant time_val(TJS_W("33"));
	DicObj->PropSetByVS(TJS_MEMBERENSURE,
		tag_name.AsVariantStringNoAddRef(), &tag_val, DicObj);
	DicObj->PropSetByVS(TJS_MEMBERENSURE,
		time_name.AsVariantStringNoAddRef(), &time_val, DicObj);
	DicObj->AddRef();
	return DicObj;
}
#endif
...
{
#ifdef __EMSCRIPTEN__
	static int gnt_count = 0;
	gnt_count++;
	if (MotionEventWaitActive) {
		if (TVPIsMotionEventBridgeWaiting()) {
			MotionEventWaitSyntheticCount++;
			if (MotionEventWaitSyntheticCount <= 8 || (MotionEventWaitSyntheticCount % 30) == 0) {
				KRKR_LOG_L2(
					"[KAG-MOTION-EV] waitmovie blocking count=%d ks=%s lbl=%s\n",
					(int)MotionEventWaitSyntheticCount,
					StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
					CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
			}
			return CreateMotionEventWaitTag();
		}
		KRKR_LOG_L2(
			"[KAG-MOTION-EV] waitmovie released count=%d ks=%s lbl=%s\n",
			(int)MotionEventWaitSyntheticCount,
			StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
			CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
		MotionEventWaitActive = false;
...
#ifdef __EMSCRIPTEN__
	// [PLAN-B] Log every tag (no cap) with common attrs + macro-stack depth.
	//          Special focus on 'motionload' and tags that follow it.
	if (result) {
		tTJSVariant tagname;
		tjs_error r = result->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("tagname"), NULL, &tagname, result);
		if (TJS_SUCCEEDED(r) && tagname.Type() == tvtString) {
			std::string tn = TVPDiagUtf8(ttstr(tagname.GetString()));

			// Probe common attrs that motion-related tags use
			auto get_str = [&](const tjs_char *k) -> std::string {
				tTJSVariant v;
				if (TJS_SUCCEEDED(result->PropGet(0, k, NULL, &v, result)) && v.Type() != tvtVoid) {
					ttstr s = v;
					return TVPDiagUtf8(s);
				}
				return std::string();
			};
			std::string storage_v = get_str(TJS_W("storage"));
			std::string target_v  = get_str(TJS_W("target"));
			std::string name_v    = get_str(TJS_W("name"));
			std::string file_v    = get_str(TJS_W("file"));
			std::string time_v    = get_str(TJS_W("time"));
			std::string layer_v   = get_str(TJS_W("layer"));
			std::string mode_v    = get_str(TJS_W("mode"));
			std::string chara_v   = get_str(TJS_W("chara"));
			std::string motion_v  = get_str(TJS_W("motion"));
			std::string wait_v    = get_str(TJS_W("wait"));
			std::string waitmovie_v = get_str(TJS_W("waitmovie"));
			std::string notrans_v = get_str(TJS_W("notrans"));
			std::string exp_v     = get_str(TJS_W("exp"));
			std::string text_v    = get_str(TJS_W("text"));

			static std::string ch_accum;
			static std::string ch_accum_storage;
			static std::string ch_accum_label;
			if (tn == "ch") {
				if (ch_accum.empty()) {
					ch_accum_storage = StorageName.GetLen() ? TVPDiagUtf8(StorageName) : "-";
					ch_accum_label = CurLabel.GetLen() ? TVPDiagUtf8(CurLabel) : "-";
				}
				ch_accum += text_v;
				if (ch_accum.size() >= 240) {
					KRKR_LOG_L2("[KAG-TEXT] ks=%s lbl=%s text=%s\n",
						ch_accum_storage.c_str(), ch_accum_label.c_str(), ch_accum.c_str());
					ch_accum.clear();
				}
				return result;
			}
			...
			KRKR_LOG_L2(
				"[KAG-TAG] #%d tag=%s mDepth=%u ks=%s lbl=%s | storage=%s target=%s name=%s file=%s time=%s layer=%s mode=%s chara=%s motion=%s wait=%s waitmovie=%s notrans=%s exp=%s text=%s\n",
				gnt_count, tn.c_str(), (unsigned)MacroArgStackDepth,
				StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
				CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-",
				storage_v.empty() ? "-" : storage_v.c_str(),
				target_v.empty() ? "-" : target_v.c_str(),
				name_v.empty() ? "-" : name_v.c_str(),
				file_v.empty() ? "-" : file_v.c_str(),
				time_v.empty() ? "-" : time_v.c_str(),
				layer_v.empty() ? "-" : layer_v.c_str(),
				mode_v.empty() ? "-" : mode_v.c_str(),
				chara_v.empty() ? "-" : chara_v.c_str(),
				motion_v.empty() ? "-" : motion_v.c_str(),
				wait_v.empty() ? "-" : wait_v.c_str(),
				waitmovie_v.empty() ? "-" : waitmovie_v.c_str(),
				notrans_v.empty() ? "-" : notrans_v.c_str(),
				exp_v.empty() ? "-" : exp_v.c_str(),
				text_v.empty() ? "-" : text_v.c_str());
		}
	} else {
		KRKR_LOG_L2("[KAG-TAG] #%d RETURNED-NULL ks=%s lbl=%s mDepth=%u\n",
			gnt_count,
			StorageName.GetLen() ? StorageName.AsNarrowStdString().c_str() : "-",
			CurLabel.GetLen() ? CurLabel.AsNarrowStdString().c_str() : "-",
			(unsigned)MacroArgStackDepth);
	}
	// Keep the header probe disabled; it is useful during archive/debug work,
	// but too noisy for the normal browser validation path.
#if 0
	// Probe motionload PSB header bytes to determine if motion PSBs are
	// pure (start with 'PSB\0') or M2-encrypted (typically 'BM..' or
	// scrambled). Read-only — does not interfere with motion playback.
	if (result) {
		tTJSVariant tn_var0;
		if (TJS_SUCCEEDED(result->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("tagname"), NULL, &tn_var0, result))
			&& tn_var0.Type() == tvtString
			&& ttstr(tn_var0.GetString()) == TJS_W("motionload")) {
			tTJSVariant sv0;
			if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("storage"), NULL, &sv0, result))
				&& sv0.Type() == tvtString) {
				ttstr storage(sv0.GetString());
				std::string sn = storage.AsNarrowStdString();
				try {
					tTJSBinaryStream *stream = TVPCreateStream(storage);
					if (stream) {
						unsigned char hdr[16] = {0};
						tjs_uint64 size = stream->GetSize();
						tjs_uint readN = stream->Read(hdr, 16);
						delete stream;
						emscripten_log(EM_LOG_CONSOLE,
							"[MOTION-PROBE] storage=%s size=%llu read=%u hdr=[%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x] ascii=[%c%c%c%c%c%c%c%c]",
							sn.c_str(), (unsigned long long)size, (unsigned)readN,
							hdr[0], hdr[1], hdr[2], hdr[3], hdr[4], hdr[5], hdr[6], hdr[7],
							hdr[8], hdr[9], hdr[10], hdr[11], hdr[12], hdr[13], hdr[14], hdr[15],
							(hdr[0] >= 32 && hdr[0] < 127) ? hdr[0] : '.',
							(hdr[1] >= 32 && hdr[1] < 127) ? hdr[1] : '.',
							(hdr[2] >= 32 && hdr[2] < 127) ? hdr[2] : '.',
							(hdr[3] >= 32 && hdr[3] < 127) ? hdr[3] : '.',
							(hdr[4] >= 32 && hdr[4] < 127) ? hdr[4] : '.',
							(hdr[5] >= 32 && hdr[5] < 127) ? hdr[5] : '.',
							(hdr[6] >= 32 && hdr[6] < 127) ? hdr[6] : '.',
							(hdr[7] >= 32 && hdr[7] < 127) ? hdr[7] : '.');
					} else {
						emscripten_log(EM_LOG_CONSOLE | EM_LOG_ERROR,
							"[MOTION-PROBE] storage=%s -> stream is null", sn.c_str());
					}
				} catch(eTJS &e) {
					emscripten_log(EM_LOG_CONSOLE | EM_LOG_ERROR,
						"[MOTION-PROBE] storage=%s -> TJS exception: %s",
						sn.c_str(), e.GetMessage().AsNarrowStdString().c_str());
				} catch(...) {
					emscripten_log(EM_LOG_CONSOLE | EM_LOG_ERROR,
						"[MOTION-PROBE] storage=%s -> unknown exception", sn.c_str());
				}
			}
		}
	}
	...
	if (result) {
		tTJSVariant tn_var;
		if (TJS_SUCCEEDED(result->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("tagname"), NULL, &tn_var, result))
			&& tn_var.Type() == tvtString) {
			ttstr tagname(tn_var);
			if (tagname == TJS_W("motionload")) {
				if (TVPIsDracuBringupEnabled()) {
					TVPLogDracuKagBridge("enabled", "motionload");
					TVPEmotePlayerHidePsbLogoFallback();
				} else {
					TVPLogDracuKagBridge("disabled", "motionload");
				}
			} else if (tagname == TJS_W("ev")) {
				bool dispatch = false;
				ttstr storage, chara, motion, waitmovie, notrans;
				tTJSVariant sv, cv, mv, wmv, ntv;
				if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("storage"), NULL, &sv, result)) &&
					sv.Type() != tvtVoid) {
					storage = ttstr(sv);
					std::string sn = storage.AsLowerCase().AsNarrowStdString();
					dispatch = sn.find(".psb") != std::string::npos;
				}
				if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("chara"), NULL, &cv, result)) &&
					cv.Type() != tvtVoid) chara = ttstr(cv);
				if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("motion"), NULL, &mv, result)) &&
					mv.Type() != tvtVoid) motion = ttstr(mv);
				if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("waitmovie"), NULL, &wmv, result)) &&
					wmv.Type() != tvtVoid) {
					waitmovie = ttstr(wmv);
					dispatch = true;
				}
				if (TJS_SUCCEEDED(result->PropGet(0, TJS_W("notrans"), NULL, &ntv, result)) &&
					ntv.Type() != tvtVoid) notrans = ttstr(ntv);

				if (dispatch) {
					if (TVPIsDracuBringupEnabled()) TVPLogDracuKagBridge("enabled", storage.GetLen() ? "ev-psb" : "ev-waitmovie");
					bool ok = TVPDispatchMotionEventBridge(result, StorageName, CurLabel);
					if (waitmovie.GetLen() && ok && TVPIsMotionEventBridgeWaiting()) {
						MotionEventWaitActive = true;
						MotionEventWaitSyntheticCount = 0;
						KRKR_LOG_L2(
							"[KAG-MOTION-EV] waitmovie armed ks=%s lbl=%s\n",
							StorageName.GetLen() ? TVPDiagUtf8(StorageName).c_str() : "-",
							CurLabel.GetLen() ? TVPDiagUtf8(CurLabel).c_str() : "-");
					}
					KRKR_LOG_L2(
						"[KAG-MOTION-EV] dispatched=%d storage=%s chara=%s motion=%s waitmovie=%s notrans=%s ks=%s lbl=%s\n",
						ok ? 1 : 0,
						storage.GetLen() ? storage.AsNarrowStdString().c_str() : "-",
						chara.GetLen() ? chara.AsNarrowStdString().c_str() : "-",
						motion.GetLen() ? motion.AsNarrowStdString().c_str() : "-",
						waitmovie.GetLen() ? waitmovie.AsNarrowStdString().c_str() : "-",
						notrans.GetLen() ? notrans.AsNarrowStdString().c_str() : "-",
						StorageName.GetLen() ? StorageName.AsNarrowStdString().c_str() : "-",
						CurLabel.GetLen() ? CurLabel.AsNarrowStdString().c_str() : "-");
				}
			}
...
// --- WebAssembly 宏重定向魔法开始 ---
  #undef TJS_NCM_REG_THIS
  #define TJS_NCM_REG_THIS classobj
  tjs_int32 ClassID = -1; // 凭空捏造一个 ClassID 给宏去赋值，防止报错
// --- WebAssembly 宏重定向魔法结束 ---
	TJS_BEGIN_NATIVE_MEMBERS(KAGParser)
	ClassID_KAGParser = TJS_NCM_CLASSID;
	printf("KAGParser ClassID assigned: %d\n", (int)ClassID_KAGParser);
...
TJS_BEGIN_NATIVE_PROP_DECL(curLine)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = _this->GetCurLine();
		return TJS_S_OK;
	}
...
TJS_BEGIN_NATIVE_PROP_DECL(curPos)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = _this->GetCurPos();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(curLineStr)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = ttstr(_this->GetCurLineStr());
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(processSpecialTags)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = (tjs_int)_this->GetProcessSpecialTags();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(ignoreCR)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = (tjs_int)_this->GetIgnoreCR();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(debugLevel)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = (tjs_int)_this->GetDebugLevel();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(macros)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		...
		if(result) *result = tTJSVariant(macros, macros);
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(macroParams)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		...
		if(result) *result = tTJSVariant(params, params);
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(callStackDepth)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = _this->GetCallStackDepth();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(curStorage)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = _this->GetStorageName();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(curLabel)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_KAGParser);
		if(result) *result = _this->GetCurLabel();
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
