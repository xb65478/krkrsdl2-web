...
static tjs_uint64 TVPAudioDiagWasmPumpTriggerCount = 0;
static tjs_uint64 TVPAudioDiagWasmPumpFillBufferCount = 0;
static tjs_uint64 TVPAudioDiagWasmPumpTriggerMsX1000 = 0;
static tjs_uint64 TVPAudioDiagWasmPumpFillBufferMsX1000 = 0;
static tjs_uint64 TVPAudioDiagWasmPumpDecodeCount = 0;
static tjs_uint64 TVPAudioDiagWasmPumpDecodeMsX1000 = 0;
static tjs_uint64 TVPAudioDiagWasmPumpReadL2Count = 0;
static tjs_uint64 TVPAudioDiagWasmPumpReadL2MsX1000 = 0;
static tjs_uint64 TVPAudioDiagWasmPumpAppendMsX1000 = 0;
static tjs_uint64 TVPAudioDiagWasmPumpLowL2RemainCount = 0;
static tjs_int TVPAudioDiagWasmPumpMinL2Remain = 0x7fffffff;

#ifdef __EMSCRIPTEN__
static double TVPAudioDiagWasmPumpLastTriggerLogMs = 0.0;
static double TVPAudioDiagWasmPumpLastFillLogMs = 0.0;
static double TVPAudioDiagWasmPumpLastReadLogMs = 0.0;
static tjs_uint64 TVPAudioDiagWasmPumpLastLoggedLowL2RemainCount = 0;
static bool TVPAudioDiagWasmBufferCtorLogged = false;

static bool TVPWasmAudioThreadPumpEnabled()
{
	...
	if(enabled < 0)
	{
		enabled = MAIN_THREAD_EM_ASM_INT({
			try {
				...
				var param = params.get("krkr_audio_thread");
				if (disabled(param)) return 0;
				var moduleValue;
				if (typeof Module !== "undefined") {
					moduleValue = Module["krkr_audio_thread"];
					if (moduleValue === undefined) moduleValue = Module["krkrAudioThread"];
				}
				if (disabled(moduleValue)) return 0;
				if (disabled(globalThis.__krkr_audio_thread)) return 0;
				return 1;
			} catch (e) {
				return 1;
			}
...
static tjs_uint64 TVPAudioPerfMsX1000(double start_ms, double end_ms)
{
	return (tjs_uint64)((end_ms - start_ms) * 1000.0);
}

static bool TVPWasmAudioClickLightStartEnabled()
{
	static int enabled = -1;
	if(enabled < 0)
	{
		enabled = MAIN_THREAD_EM_ASM_INT({
			try {
				...
				var params = new URLSearchParams(location.search || "");
				if (disabled(params.get("krkr_audio_click_light_start"))) return 0;
				var moduleValue;
				if (typeof Module !== "undefined") {
					moduleValue = Module["krkr_audio_click_light_start"];
					if (moduleValue === undefined) moduleValue = Module["krkrAudioClickLightStart"];
				}
				if (disabled(moduleValue)) return 0;
				if (disabled(globalThis.__krkr_audio_click_light_start)) return 0;
				return 1;
			} catch (e) {
				return 1;
			}
...
static const char *TVPWaveAudioSemanticStateName(tTVPWaveAudioSemanticState state)
{
	switch(state)
	{
	case tTVPWaveAudioSemanticState::Closed: return "Closed";
	case tTVPWaveAudioSemanticState::Opened: return "Opened";
	case tTVPWaveAudioSemanticState::Prefilling: return "Prefilling";
	case tTVPWaveAudioSemanticState::Playing: return "Playing";
	case tTVPWaveAudioSemanticState::Paused: return "Paused";
	case tTVPWaveAudioSemanticState::Stopping: return "Stopping";
	case tTVPWaveAudioSemanticState::Stopped: return "Stopped";
	case tTVPWaveAudioSemanticState::Draining: return "Draining";
	case tTVPWaveAudioSemanticState::Error: return "Error";
	default: return "unknown";
	}
}

const char *tTJSNI_WaveSoundBuffer::GetAudioSemanticStateName() const
{
	return TVPWaveAudioSemanticStateName(AudioSemanticState);
}

void tTJSNI_WaveSoundBuffer::SetAudioSemanticState(tTVPWaveAudioSemanticState state, const char *reason)
{
	if (AudioSemanticState == state && !TVPPerfEnabled())
		return;
	AudioSemanticState = state;
	LogAudioSemanticState(reason ? reason : "state");
}

void tTJSNI_WaveSoundBuffer::LogAudioSemanticState(const char *phase)
{
	if (!TVPPerfEnabled())
		return;
	tjs_int l2remain = -1;
	{
		tTJSCriticalSectionHolder remain_holder(L2BufferRemainCS);
		l2remain = L2BufferRemain;
	}
	fprintf(stderr,
		"[PERF] av.state component=wave phase=%s storage=%s state=%s generation=%llu status=%d buffer_playing=%d ds_playing=%d paused=%d sink_id=%d backend=%s sink_state=%s sink_generation=%llu sink_played_samples=%llu ring_fill_bytes=%d underrun=%d l2_remain=%d decode_pos=%lld\n",
		phase ? phase : "state",
		CurrentStorageName.AsNarrowStdString().c_str(),
		GetAudioSemanticStateName(),
		(unsigned long long)AudioSemanticGeneration,
		(int)Status,
		BufferPlaying ? 1 : 0,
		DSBufferPlaying ? 1 : 0,
		Paused ? 1 : 0,
		SoundBuffer ? SoundBuffer->GetDebugSinkId() : 0,
		SoundBuffer ? SoundBuffer->GetDebugBackendName() : "none",
		SoundBuffer ? SoundBuffer->GetDebugStateName() : "none",
		SoundBuffer ? (unsigned long long)SoundBuffer->GetDebugGeneration() : 0,
		SoundBuffer ? (unsigned long long)SoundBuffer->GetDebugPlayedSamples64() : 0,
		SoundBuffer ? SoundBuffer->GetDebugRingFillBytes() : -1,
		SoundBuffer ? SoundBuffer->GetDebugUnderrunCount() : -1,
		(int)l2remain,
		(long long)DecodePos);
}


extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpTriggerCount()
{
	return TVPAudioDiagWasmPumpTriggerCount;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpFillBufferCount()
{
	return TVPAudioDiagWasmPumpFillBufferCount;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpTriggerMsX1000()
{
	return TVPAudioDiagWasmPumpTriggerMsX1000;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpFillBufferMsX1000()
{
	return TVPAudioDiagWasmPumpFillBufferMsX1000;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpDecodeCount()
{
	return TVPAudioDiagWasmPumpDecodeCount;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpDecodeMsX1000()
{
	return TVPAudioDiagWasmPumpDecodeMsX1000;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpReadL2Count()
{
	return TVPAudioDiagWasmPumpReadL2Count;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpReadL2MsX1000()
{
	return TVPAudioDiagWasmPumpReadL2MsX1000;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpAppendMsX1000()
{
	return TVPAudioDiagWasmPumpAppendMsX1000;
}

extern "C" tjs_uint64 KRKRSDL2AudioDiagGetWasmPumpLowL2RemainCount()
{
	return TVPAudioDiagWasmPumpLowL2RemainCount;
}

extern "C" tjs_int KRKRSDL2AudioDiagGetWasmPumpMinL2Remain()
{
	return TVPAudioDiagWasmPumpMinL2Remain == 0x7fffffff ? -1 : TVPAudioDiagWasmPumpMinL2Remain;
}
//---------------------------------------------------------------------------
...
public:
	...
	bool IsThreadPumpActive() const { return ThreadPumpActive; }
...
tTVPWaveSoundBufferThread::tTVPWaveSoundBufferThread()
	...
	NextLabelEventTick( 0 ), LastFilledTick( 0 ), ThreadPumpActive( false ),
	WndProcToBeCalled( false )
{
	EventQueue.Allocate();
	SetPriority(ttpHighest);
#ifdef __EMSCRIPTEN__
	ThreadPumpActive = TVPWasmAudioThreadPumpEnabled();
	if(ThreadPumpActive)
	{
		fprintf(stderr, "[DIAG-SND] WaveSoundBuffer wasm pthread audio pump enabled pthreads=1\n");
		fflush(stderr);
		try {
			StartTread();
		}
		...
		{
			ThreadPumpActive = false;
			fprintf(stderr, "[DIAG-SND] WaveSoundBuffer wasm pthread audio pump failed; falling back to main-loop pump\n");
			fflush(stderr);
		}
	}
	else
	{
#ifdef __EMSCRIPTEN_PTHREADS__
		fprintf(stderr, "[DIAG-SND] WaveSoundBuffer wasm pthread audio pump disabled; using main-loop pump pthreads=1\n");
#else
		fprintf(stderr, "[DIAG-SND] WaveSoundBuffer wasm main-loop audio pump selected pthreads=0\n");
#endif
		fflush(stderr);
	}
#else
	ThreadPumpActive = true;
	StartTread();
#endif
}
//---------------------------------------------------------------------------
...
#ifdef __EMSCRIPTEN__
#define TVP_WSB_WASM_PREFILL_OPENAL_BUFFERS 4
#define TVP_WSB_WASM_PREFILL_L2_UNITS 4
#define TVP_WSB_WASM_PUMP_MAX_FILLS 3
#define TVP_WSB_WASM_PUMP_BUDGET_MS 3.0
#endif
...
void tTVPWaveSoundBufferThread::InternalTrigger()
{
	if(ThreadPumpActive)
		return;
	TVPAudioDiagWasmPumpTriggerCount++;
		...
		{	// thread-protected
			...
			if (TVPPrimarySoundBufferPlaying && time - LastFilledTick >= TVP_WSB_THREAD_SLEEP_TIME)
			{
					...
					i != TVPWaveSoundBufferVector.end(); i++)
				{
					if((*i)->ThreadCallbackEnabled)
					{
						for (int fill = 0; fill < TVP_WSB_WASM_PUMP_MAX_FILLS; fill++)
						{
							TVPAudioDiagWasmPumpFillBufferCount++;
							double fill_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
							(*i)->PrefillL2BufferForWasm(TVP_WSB_WASM_PREFILL_L2_UNITS, 1.5);
							bool wait = (*i)->FillBuffer(); // fill sound buffer
							double fill_end_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
							if (TVPPerfEnabled())
							{
								TVPAudioDiagWasmPumpFillBufferMsX1000 += TVPAudioPerfMsX1000(fill_start_ms, fill_end_ms);
								if (fill_end_ms - perf_start_ms >= TVP_WSB_WASM_PUMP_BUDGET_MS)
									break;
							}
							if (wait)
								break;
	...
	if (TVPPerfEnabled())
	{
		...
		TVPAudioDiagWasmPumpTriggerMsX1000 += TVPAudioPerfMsX1000(perf_start_ms, perf_end_ms);
		bool printSample = perf_end_ms - perf_start_ms >= 4.0 ||
			TVPAudioDiagWasmPumpTriggerCount <= 4 ||
			(TVPAudioDiagWasmPumpLastTriggerLogMs > 0.0 && perf_end_ms - TVPAudioDiagWasmPumpLastTriggerLogMs >= 1000.0);
		if (printSample)
		{
			TVPAudioDiagWasmPumpLastTriggerLogMs = perf_end_ms;
			fprintf(stderr,
				"[PERF] audio.trigger count=%llu trigger_ms=%.3f fill_count=%llu fill_ms=%.3f min_l2_remain=%d low_l2_count=%llu\n",
				(unsigned long long)TVPAudioDiagWasmPumpTriggerCount,
				perf_end_ms - perf_start_ms,
				(unsigned long long)TVPAudioDiagWasmPumpFillBufferCount,
				(double)TVPAudioDiagWasmPumpFillBufferMsX1000 / 1000.0,
				(int)KRKRSDL2AudioDiagGetWasmPumpMinL2Remain(),
				(unsigned long long)TVPAudioDiagWasmPumpLowL2RemainCount);
		}
	}
}
#endif
...
tTJSNI_WaveSoundBuffer::tTJSNI_WaveSoundBuffer()
{
	...
	SoundBuffer = NULL;
	AudioSemanticState = tTVPWaveAudioSemanticState::Closed;
	AudioSemanticGeneration = 1;
#if 0
	Sound3DBuffer = NULL;
#endif
	...
	L2BufferUnits = 0;
#ifdef __EMSCRIPTEN__
	if(!TVPAudioDiagWasmBufferCtorLogged)
	{
		fprintf(stderr, "[DIAG-SND] WaveSoundBuffer wasm buffer constructed; pump mode will be selected on first play\n");
		fflush(stderr);
		TVPAudioDiagWasmBufferCtorLogged = true;
	}
#endif
...
void tTJSNI_WaveSoundBuffer::TryCreateSoundBuffer(bool use3d)
{
	...
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Opened, "sink-create");
#if 0
	// setup parameters
	DSBUFFERDESC dsbd;
	ZeroMemory(&dsbd, sizeof(dsbd));
...
void tTJSNI_WaveSoundBuffer::DestroySoundBuffer()
{
	...
	if(SoundBuffer)
	{
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Stopping, "destroy-begin");
		SoundBuffer->Stop();
		SoundBuffer->Release();
		SoundBuffer = NULL;
	}
	...
	BufferPlaying = false;
	AudioSemanticGeneration++;
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Closed, "destroy");
...
void tTJSNI_WaveSoundBuffer::ResetSoundBuffer()
{
	if(SoundBuffer)
	{
		SoundBuffer->Reset();
		AudioSemanticGeneration++;
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Stopped, "reset-buffer");
#if 0
...
void tTJSNI_WaveSoundBuffer::Clear()
{
	...
	if(!Thread)
	{
		...
		ResetSamplePositions();
		SetStatus(ssUnload);
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Closed, "clear-no-thread");
		return;
	}
#endif
	Stop();
	...
	SetStatus(ssUnload);
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Closed, "clear");
}
//---------------------------------------------------------------------------
...
{
	if(TVPWaveSoundBufferThread && !TVPWaveSoundBufferThread->IsThreadPumpActive())
...
bool tTJSNI_WaveSoundBuffer::FillL2Buffer(bool firstwrite, bool fromdecodethread)
{
#ifdef __EMSCRIPTEN__
	double perf_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
	tjs_int perf_remain_before = -1;
	if (TVPPerfEnabled())
	{
		tTJSCriticalSectionHolder remain_holder(L2BufferRemainCS);
		perf_remain_before = L2BufferRemain;
		if (perf_remain_before < TVPAudioDiagWasmPumpMinL2Remain)
			TVPAudioDiagWasmPumpMinL2Remain = perf_remain_before;
		if (perf_remain_before <= 1)
			TVPAudioDiagWasmPumpLowL2RemainCount++;
	}
#endif
	if(!fromdecodethread && Thread && Thread->GetRunning())
...

	{
		...
		if(Thread && Thread->GetRunning()) Thread->SetPriority(ttpbefore);
	...
	else
	{
		L2BufferSegmentQueues[L2BufferWritePos].Clear();
#ifdef __EMSCRIPTEN__
		double decode_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
#endif
		...
		if (TVPPerfEnabled())
		{
			TVPAudioDiagWasmPumpDecodeCount++;
			TVPAudioDiagWasmPumpDecodeMsX1000 += TVPAudioPerfMsX1000(decode_start_ms, emscripten_get_now());
		}
#endif
	...
	if (TVPPerfEnabled())
	{
		double perf_end_ms = emscripten_get_now();
		bool printSample = perf_end_ms - perf_start_ms >= 4.0 ||
			TVPAudioDiagWasmPumpDecodeCount <= 4 ||
			(TVPAudioDiagWasmPumpLastFillLogMs > 0.0 && perf_end_ms - TVPAudioDiagWasmPumpLastFillLogMs >= 1000.0);
		if (printSample)
		{
			TVPAudioDiagWasmPumpLastFillLogMs = perf_end_ms;
			tjs_int remain_after = -1;
			{
				tTJSCriticalSectionHolder remain_holder(L2BufferRemainCS);
				remain_after = L2BufferRemain;
			}
			fprintf(stderr,
				"[PERF] audio.fill_l2 first=%d from_thread=%d remain_before=%d remain_after=%d ms=%.3f decode_count=%llu decode_ms=%.3f low_l2_count=%llu\n",
				firstwrite ? 1 : 0,
				fromdecodethread ? 1 : 0,
				(int)perf_remain_before,
				(int)remain_after,
				perf_end_ms - perf_start_ms,
				(unsigned long long)TVPAudioDiagWasmPumpDecodeCount,
				(double)TVPAudioDiagWasmPumpDecodeMsX1000 / 1000.0,
				(unsigned long long)TVPAudioDiagWasmPumpLowL2RemainCount);
		}
	}
#endif
...
void tTJSNI_WaveSoundBuffer::PrepareToReadL2Buffer(bool firstread)
{
	...
	if(Thread && Thread->GetRunning()) Thread->SetPriority(TVPDecodeThreadHighPriority);
			// make decoder thread priority higher than usual,
			// before entering critical section
}
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
void tTJSNI_WaveSoundBuffer::PrefillL2BufferForWasm(tjs_int target_units, double budget_ms)
{
	double start_ms = emscripten_get_now();
	while(!L2BufferEnded)
	{
		tjs_int remain = 0;
		{
		...
		if(remain >= target_units || remain >= L2BufferUnits)
			break;
		if(!FillL2Buffer(false, false))
			break;
...
//---------------------------------------------------------------------------
bool tTJSNI_WaveSoundBuffer::EnsureL2BufferForWasm()
{
	tjs_int remain = 0;
	{
		tTJSCriticalSectionHolder holder(L2BufferRemainCS);
		...
		tTVPWaveSegmentQueue & segments)
{
	...
	if (decoded) {
#ifdef __EMSCRIPTEN__
		append_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
#endif
...
#ifdef __EMSCRIPTEN__
		if (TVPPerfEnabled())
			TVPAudioDiagWasmPumpAppendMsX1000 += TVPAudioPerfMsX1000(append_start_ms, emscripten_get_now());
#endif
	...
	{
		double perf_end_ms = emscripten_get_now();
		TVPAudioDiagWasmPumpReadL2Count++;
		TVPAudioDiagWasmPumpReadL2MsX1000 += TVPAudioPerfMsX1000(perf_start_ms, perf_end_ms);
		bool lowL2Changed = TVPAudioDiagWasmPumpLowL2RemainCount != TVPAudioDiagWasmPumpLastLoggedLowL2RemainCount;
		bool printSample = perf_end_ms - perf_start_ms >= 4.0 ||
			TVPAudioDiagWasmPumpReadL2Count <= 4 ||
			(lowL2Changed && TVPAudioDiagWasmPumpLowL2RemainCount <= 4) ||
			(lowL2Changed && TVPAudioDiagWasmPumpLowL2RemainCount % 128 == 0) ||
			(TVPAudioDiagWasmPumpLastReadLogMs > 0.0 && perf_end_ms - TVPAudioDiagWasmPumpLastReadLogMs >= 1000.0);
		if (printSample)
		{
			TVPAudioDiagWasmPumpLastReadLogMs = perf_end_ms;
			TVPAudioDiagWasmPumpLastLoggedLowL2RemainCount = TVPAudioDiagWasmPumpLowL2RemainCount;
			tjs_int remain_after = -1;
			{
				tTJSCriticalSectionHolder remain_holder(L2BufferRemainCS);
				remain_after = L2BufferRemain;
			...
			fprintf(stderr,
				"[PERF] audio.read_l2 decoded=%u bytes=%u remain_before=%d remain_after=%d ms=%.3f append_total_ms=%.3f low_l2_count=%llu\n",
				(unsigned)decoded,
				(unsigned)(decoded * InputFormat.BytesPerSample * InputFormat.Channels),
				(int)perf_remain_before,
				(int)remain_after,
				perf_end_ms - perf_start_ms,
				(double)TVPAudioDiagWasmPumpAppendMsX1000 / 1000.0,
				(unsigned long long)TVPAudioDiagWasmPumpLowL2RemainCount);
		}
	}
#endif
...
bool tTJSNI_WaveSoundBuffer::FillBuffer(bool firstwrite, bool allowpause)
{
	...
	if(Thread && Thread->GetRunning() && bufferremain < TVP_WSB_ACCESS_FREQ )
...
void tTJSNI_WaveSoundBuffer::ResetLastCheckedDecodePos(DWORD pp)
{
	...
	tjs_uint current = SoundBuffer->GetCurrentPlaySamples();
	tjs_int rblock = AccessUnitSamples > 0 && L1BufferUnits > 0 ?
		static_cast<tjs_int>((current / AccessUnitSamples) % L1BufferUnits) : 0;
	tjs_int offset = AccessUnitSamples > 0 ?
		static_cast<tjs_int>(current % AccessUnitSamples) : 0;
...
void tTJSNI_WaveSoundBuffer::StartPlay()
{
...
#ifdef __EMSCRIPTEN__
	double perf_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
	double after_ensure_primary_ms = perf_start_ms;
	double after_ensure_thread_ms = perf_start_ms;
	double after_create_ms = perf_start_ms;
	double after_reset_ms = perf_start_ms;
	double after_fill_l2_ms = perf_start_ms;
	double after_fill_buffer_ms = perf_start_ms;
	double after_prefill_l2_ms = perf_start_ms;
	double after_prefill_openal_ms = perf_start_ms;
	bool lightStart = TVPWasmAudioClickLightStartEnabled();
	bool threadPumpActive = false;
	bool didSyncPrefill = false;
	int openalPrefillCount = 0;
	int l2RemainAfterStart = -1;
	int queuedAfterStart = -1;
	int sinkIdAfterStart = 0;
	const char *backendAfterStart = "unknown";
	int ringFillAfterStart = -1;
	int underrunAfterStart = -1;
#endif
#ifdef __EMSCRIPTEN__
	if(!Thread)
	{
		SetStatus(ssPlay);
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Error, "start-no-thread");
...
#ifdef __EMSCRIPTEN__
	if(TVPPerfEnabled()) after_ensure_primary_ms = emscripten_get_now();
#endif

	// ensure playing thread
	TVPEnsureWaveSoundBufferWorking();
#ifdef __EMSCRIPTEN__
	threadPumpActive = TVPWaveSoundBufferThread && TVPWaveSoundBufferThread->IsThreadPumpActive();
	if(TVPPerfEnabled()) after_ensure_thread_ms = emscripten_get_now();
#endif
	...
	{	// thread protected block
		...
		CreateSoundBuffer();
#ifdef __EMSCRIPTEN__
		if(TVPPerfEnabled()) after_create_ms = emscripten_get_now();
#endif

		// reset filter chain
		ResetFilterChain();
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Prefilling, "start-prefill");
#ifdef __EMSCRIPTEN__
		if(TVPPerfEnabled()) after_reset_ms = emscripten_get_now();
#endif
...
#ifdef __EMSCRIPTEN__
		if(TVPPerfEnabled()) after_fill_l2_ms = emscripten_get_now();
#endif
		FillBuffer(true, false);
#ifdef __EMSCRIPTEN__
		if(TVPPerfEnabled()) after_fill_buffer_ms = emscripten_get_now();
#endif
#ifdef __EMSCRIPTEN__
		bool wantsStartPrefill = SoundBuffer && SoundBuffer->WantsStartPrefill();
		if(!lightStart || wantsStartPrefill)
		{
			didSyncPrefill = true;
			double prefillBudgetMs = wantsStartPrefill ? 4.0 : 6.0;
			tjs_int prefillTarget = wantsStartPrefill ? 4 : TVP_WSB_WASM_PREFILL_L2_UNITS;
			PrefillL2BufferForWasm(prefillTarget, prefillBudgetMs);
			if(TVPPerfEnabled()) after_prefill_l2_ms = emscripten_get_now();
			int targetQueued = wantsStartPrefill ? 4 : TVP_WSB_WASM_PREFILL_OPENAL_BUFFERS;
			double fillBudgetStartMs = emscripten_get_now();
			for (int i = 1; i < targetQueued; i++)
			{
				openalPrefillCount++;
				if (FillBuffer(false, false))
					break;
				if (wantsStartPrefill && emscripten_get_now() - fillBudgetStartMs >= 4.0)
					break;
			}
			if(TVPPerfEnabled()) after_prefill_openal_ms = emscripten_get_now();
		}
		else
		{
			after_prefill_l2_ms = after_fill_buffer_ms;
			after_prefill_openal_ms = after_fill_buffer_ms;
		}
#else
#if 0
		...
		if(!Paused)
		{
			SoundBuffer->Play(/*0, 0, DSBPLAY_LOOPING*/);
			DSBufferPlaying = true;
			SetAudioSemanticState(tTVPWaveAudioSemanticState::Playing, "start-play");
		}
		else
		{
			SetAudioSemanticState(tTVPWaveAudioSemanticState::Paused, "start-paused");
		}
	...
	if(TVPPerfEnabled())
	{
		{
			tTJSCriticalSectionHolder remain_holder(L2BufferRemainCS);
			l2RemainAfterStart = L2BufferRemain;
		}
		queuedAfterStart = SoundBuffer ? SoundBuffer->GetRemainBuffers() : -1;
		sinkIdAfterStart = SoundBuffer ? SoundBuffer->GetDebugSinkId() : 0;
		backendAfterStart = SoundBuffer ? SoundBuffer->GetDebugBackendName() : "none";
		ringFillAfterStart = SoundBuffer ? SoundBuffer->GetDebugRingFillBytes() : -1;
		underrunAfterStart = SoundBuffer ? SoundBuffer->GetDebugUnderrunCount() : -1;
		double perf_end_ms = emscripten_get_now();
		fprintf(stderr,
			"[PERF] audio.start_play storage=%s state=%s generation=%llu sink_id=%d backend=%s sink_state=%s sink_generation=%llu sink_played_samples=%llu total_ms=%.3f ensure_primary_ms=%.3f ensure_thread_ms=%.3f create_ms=%.3f reset_ms=%.3f fill_l2_ms=%.3f fill_buffer_ms=%.3f prefill_l2_ms=%.3f prefill_output_ms=%.3f light_start=%d thread_pump=%d sync_prefill=%d output_prefill=%d l2_remain=%d queued=%d ring_fill_bytes=%d underrun=%d\n",
			CurrentStorageName.AsNarrowStdString().c_str(),
			GetAudioSemanticStateName(),
			(unsigned long long)AudioSemanticGeneration,
			sinkIdAfterStart,
			...
			SoundBuffer ? (unsigned long long)SoundBuffer->GetDebugPlayedSamples64() : 0,
			perf_end_ms - perf_start_ms,
			after_ensure_primary_ms - perf_start_ms,
			after_ensure_thread_ms - after_ensure_primary_ms,
			after_create_ms - after_ensure_thread_ms,
			after_reset_ms - after_create_ms,
			after_fill_l2_ms - after_reset_ms,
			after_fill_buffer_ms - after_fill_l2_ms,
			after_prefill_l2_ms - after_fill_buffer_ms,
			after_prefill_openal_ms - after_prefill_l2_ms,
			lightStart ? 1 : 0,
			threadPumpActive ? 1 : 0,
			didSyncPrefill ? 1 : 0,
			openalPrefillCount,
			l2RemainAfterStart,
			queuedAfterStart,
...
void tTJSNI_WaveSoundBuffer::StopPlay()
{
	...
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Stopping, "stop-play-begin");
	SoundBuffer->Stop();
	DSBufferPlaying = false;
	BufferPlaying = false;
	AudioSemanticGeneration++;
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Stopped, "stop-play");
...
void tTJSNI_WaveSoundBuffer::Play()
{
	...
	if(!Decoder) {
		SetStatus(ssPlay);
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Error, "play-no-decoder");
		SetStatusAsync(ssStop);
		return;
	}
#else
...
void tTJSNI_WaveSoundBuffer::Stop()
{
	...
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Stopped, "stop");
...
void tTJSNI_WaveSoundBuffer::SetBufferPaused(bool bPaused) {
	...
	if (bPaused)
	{
		SoundBuffer->Pause();
		DSBufferPlaying = false;
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Paused, "primary-pause");
	}
	else { // restore
		if (!Paused && BufferPlaying) {
			SoundBuffer->Play();
			DSBufferPlaying = true;
			SetAudioSemanticState(tTVPWaveAudioSemanticState::Playing, "primary-resume");
		}
	}
}
	...
	if(SoundBuffer && BufferPlaying)
	{
		if(Paused)
		{
			if(DSBufferPlaying)
			{
				SoundBuffer->Pause();
				DSBufferPlaying = false;
				SetAudioSemanticState(tTVPWaveAudioSemanticState::Paused, "pause");
			}
		}
		else if(!DSBufferPlaying && TVPPrimarySoundBufferPlaying)
		{
			SoundBuffer->Play();
			DSBufferPlaying = true;
			SetAudioSemanticState(tTVPWaveAudioSemanticState::Playing, "resume");
		}
	}
	else
	{
		SetAudioSemanticState(Paused ? tTVPWaveAudioSemanticState::Paused :
			tTVPWaveAudioSemanticState::Stopped, "pause-state");
	}
}
//---------------------------------------------------------------------------
...
void tTJSNI_WaveSoundBuffer::Open(const ttstr & storagename)
{
	...
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Opened, "open-begin");
#ifdef __EMSCRIPTEN__
	double perf_start_ms = TVPPerfEnabled() ? emscripten_get_now() : 0.0;
	double after_primary_ms = perf_start_ms;
	double after_clear_ms = perf_start_ms;
	double after_decoder_ms = perf_start_ms;
	double after_loop_ms = perf_start_ms;
	double after_filter_ms = perf_start_ms;
	double after_format_ms = perf_start_ms;
	std::string perf_storage_name;
	if(TVPPerfEnabled())
		perf_storage_name = storagename.AsNarrowStdString();
#endif
	TVPEnsurePrimaryBufferPlay(); // let primary buffer to start running
#ifdef __EMSCRIPTEN__
	if(TVPPerfEnabled()) after_primary_ms = emscripten_get_now();
#endif

	Clear();
	CurrentStorageName = storagename;
#ifdef __EMSCRIPTEN__
	if(TVPPerfEnabled()) after_clear_ms = emscripten_get_now();
#endif
...
#ifdef __EMSCRIPTEN__
	if(TVPPerfEnabled()) after_decoder_ms = emscripten_get_now();
#endif
#ifdef __EMSCRIPTEN__
	} catch(...) {
			fprintf(stderr, "[STUB-P1] dll=WaveSoundBuffer fallback=silent-audio missing=decoder/playback target_specific=no action=decoder-open-failed storage=%s\n",
				storagename.AsNarrowStdString().c_str());
		Decoder = NULL;
		SetStatus(ssStop);
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Error, "open-decoder-failed");
		return;
	}
#endif
...
#ifdef __EMSCRIPTEN__
			if(TVPPerfEnabled()) after_loop_ms = emscripten_get_now();
#endif

			// build filter chain
			RebuildFilterChain();
#ifdef __EMSCRIPTEN__
			if(TVPPerfEnabled()) after_filter_ms = emscripten_get_now();
#endif
...
#ifdef __EMSCRIPTEN__
			if(TVPPerfEnabled()) after_format_ms = emscripten_get_now();
#endif
		}
		catch(...)
		{
	...
	// set status to stop
	SetStatus(ssStop);
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Opened, "open-ready");
#ifdef __EMSCRIPTEN__
	if(TVPPerfEnabled())
	{
		double perf_end_ms = emscripten_get_now();
		fprintf(stderr,
			"[PERF] audio.open storage=%s state=%s generation=%llu sink_id=%d backend=%s sink_state=%s sink_generation=%llu ring_fill_bytes=%d underrun=%d total_ms=%.3f ensure_primary_ms=%.3f clear_ms=%.3f decoder_ms=%.3f loop_ms=%.3f filter_ms=%.3f format_ms=%.3f sli_ms=%.3f rate=%d ch=%d bits=%d total_time=%llu\n",
			perf_storage_name.c_str(),
			GetAudioSemanticStateName(),
			(unsigned long long)AudioSemanticGeneration,
			SoundBuffer ? SoundBuffer->GetDebugSinkId() : 0,
			SoundBuffer ? SoundBuffer->GetDebugBackendName() : "none",
			...
			SoundBuffer ? SoundBuffer->GetDebugUnderrunCount() : -1,
			perf_end_ms - perf_start_ms,
			after_primary_ms - perf_start_ms,
			after_clear_ms - after_primary_ms,
			after_decoder_ms - after_clear_ms,
			after_loop_ms - after_decoder_ms,
			after_filter_ms - after_loop_ms,
			after_format_ms - after_filter_ms,
			perf_end_ms - after_format_ms,
			(int)InputFormat.SamplesPerSec,
			(int)InputFormat.Channels,
			(int)InputFormat.BitsPerSample,
...
tjs_uint64 tTJSNI_WaveSoundBuffer::GetSamplePosition()
{
	...
	tjs_uint64 pos = segs.GetFilteredLength() > 0 ?
		segs.FilteredPositionToDecodePosition(offset) : current;
	if(InputFormat.TotalSamples && pos > InputFormat.TotalSamples)
		pos = InputFormat.TotalSamples;
	return pos;
}
//---------------------------------------------------------------------------
void tTJSNI_WaveSoundBuffer::SetSamplePosition(tjs_uint64 pos)
{
	...
	SetAudioSemanticState(tTVPWaveAudioSemanticState::Prefilling, "seek-begin");
	if(BufferPlaying && DSBufferPlaying)
	{
		StopPlay();
		if(LoopManager) LoopManager->SetPosition(possamples);
		StartPlay();
	}
	else
	{
		if(LoopManager) LoopManager->SetPosition(possamples);
		SetAudioSemanticState(tTVPWaveAudioSemanticState::Opened, "seek");
	}
}
//---------------------------------------------------------------------------
...
{
	if(InputFormat.SamplesPerSec == 0) return 0L;
