...
enum class tTVPWaveAudioSemanticState
{
	Closed,
	Opened,
	Prefilling,
...
private:
	...
	ttstr CurrentStorageName;
	tTVPWaveAudioSemanticState AudioSemanticState;
	tjs_uint64 AudioSemanticGeneration;
#if 0
	LPDIRECTSOUND3DBUFFER Sound3DBuffer;
#endif

	void ThrowSoundBufferException(const ttstr &reason);
	void SetAudioSemanticState(tTVPWaveAudioSemanticState state, const char *reason);
	const char *GetAudioSemanticStateName() const;
	void LogAudioSemanticState(const char *phase);
...
#ifdef __EMSCRIPTEN__
	void PrefillL2BufferForWasm(tjs_int target_units, double budget_ms);
	bool EnsureL2BufferForWasm();
#endif
