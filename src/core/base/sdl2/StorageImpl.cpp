...
//---------------------------------------------------------------------------
bool TVPCheckExistentStorage(const ttstr &name)
{
	if(name.IsEmpty()) return false;
	const tjs_char *p = name.c_str();
	if(TJS_strncmp(p, TJS_W("http://"), 7) == 0 ||
	   TJS_strncmp(p, TJS_W("https://"), 8) == 0) {
		return true;
	}
...
tTVPLocalFileStream::~tTVPLocalFileStream()
{
	...
	if (written)
	{
		...
		std::string sync_path = localname_.AsNarrowStdString();
		if (sync_path.rfind("/tmp/", 0) == 0 || sync_path.rfind("file:///tmp/", 0) == 0)
		{
			KRKR_LOG_L2("[IDBFS-SYNC] skip-temp path=%s\n", sync_path.c_str());
			return;
		}
		KRKR_LOG_L2("[IDBFS-SYNC] request-source=LocalFileStream path=%s\n", sync_path.c_str());
#endif
		Application->SyncSavedata();
	}
}
...
EM_JS(int, emscripten_sync_http_read, (const char* url_ptr, uint32_t offset, uint32_t size, void* buffer_ptr, uint32_t chunk_size, int strategy, const char* item_key_ptr), {
	const url = UTF8ToString(url_ptr);
	const requestedChunkSize = chunk_size >>> 0;
	const readStrategy = strategy | 0;
	const itemKey = item_key_ptr ? UTF8ToString(item_key_ptr) : "";
	if (typeof WorkerGlobalScope === 'undefined') {
		try {
			...
			if (xhr.status === 206 || xhr.status === 200) {
				...
				for (let i = 0; i < n; i++) HEAPU8[buffer_ptr + i] = text.charCodeAt(i) & 0xff;
				return n;
			}
			console.error("[JS Bridge] sync main-thread read HTTP " + xhr.status + " for " + url);
		} catch (e) {
			console.error("[JS Bridge] sync main-thread read failed for " + url + ": " + e.message);
		}
		return 0;
	}
	
	if (!self.ioSab) {
		self.ioSab = new SharedArrayBuffer(1024 * 1024 * 4 + 16); // 4MB + 16 bytes
		self.ioInt32 = new Int32Array(self.ioSab);
		self.ioFloat64 = new Float64Array(self.ioSab);
		self.ioUint8 = new Uint8Array(self.ioSab, 16);
	}
	
	if (size > 1024 * 1024 * 4) size = 1024 * 1024 * 4;
	...
	Atomics.wait(self.ioInt32, 0, 0);
	
	if (self.ioInt32[0] === 1) {
		const read_size = self.ioInt32[1];
		HEAPU8.set(self.ioUint8.subarray(0, read_size), buffer_ptr);
		return read_size;
	}
	return 0;
});

EM_JS(uint64_t, emscripten_sync_http_get_size, (const char* url_ptr), {
	const url = UTF8ToString(url_ptr);
	if (typeof WorkerGlobalScope === 'undefined') {
		try {
			const xhr = new XMLHttpRequest();
			...
			if (xhr.status >= 200 && xhr.status < 400) {
				return BigInt(Number(xhr.getResponseHeader('Content-Length')) || 0);
			}
			console.error("[JS Bridge] sync main-thread size HTTP " + xhr.status + " for " + url);
		} catch (e) {
			console.error("[JS Bridge] sync main-thread size failed for " + url + ": " + e.message);
		}
		return 0n;
	}
	
	if (!self.ioSab) {
		self.ioSab = new SharedArrayBuffer(1024 * 1024 * 2 + 16); // 2MB + 16 bytes
		self.ioInt32 = new Int32Array(self.ioSab);
		self.ioFloat64 = new Float64Array(self.ioSab);
		self.ioUint8 = new Uint8Array(self.ioSab, 16);
	}
	...
	if (self.ioInt32[0] === 1) {
		return BigInt(self.ioFloat64[1]);
	}
	return 0n;
});

EM_JS(int, emscripten_sync_http_check_exist, (const char* url_ptr), {
	const url = UTF8ToString(url_ptr);
	if (globalThis.__krkr_loglevel >= 3) console.log("[JS Bridge] krkr_io_exist called for: " + url);
	if (typeof WorkerGlobalScope === 'undefined') {
		try {
			const xhr = new XMLHttpRequest();
			xhr.open('HEAD', url, false);
			...
			if (globalThis.__krkr_loglevel >= 2) console.log("[JS Bridge] sync main-thread exist HTTP " + xhr.status + " for " + url);
		} catch (e) {
			console.error("[JS Bridge] sync main-thread exist failed for " + url + ": " + e.message);
		}
		return 0;
	}
	...
	return self.ioInt32[0] === 1 ? 1 : 0;
});

EM_JS(int, emscripten_sync_http_list, (const char* url_ptr, char* out_buf, int max_len), {
	if (globalThis.__krkr_loglevel >= 3) console.log("[JS Bridge] krkr_io_list called");
	if (typeof WorkerGlobalScope === 'undefined') {
		console.error("WorkerGlobalScope undefined");
		return 0;
	}
	const url = UTF8ToString(url_ptr);
	if (globalThis.__krkr_loglevel >= 3) console.log("[JS Bridge] list url: " + url);
	if (!self.ioSab) {
		self.ioSab = new SharedArrayBuffer(1024 * 1024 * 2 + 16); // 2MB + 16 bytes
		self.ioInt32 = new Int32Array(self.ioSab);
		self.ioFloat64 = new Float64Array(self.ioSab);
	...
	if (self.ioInt32[0] === 1) {
		const len = self.ioInt32[1];
		if (globalThis.__krkr_loglevel >= 3) console.log("[JS Bridge] list success, len: " + len);
		const copyLen = len < max_len - 1 ? len : max_len - 1;
		HEAPU8.set(self.ioUint8.subarray(0, copyLen), out_buf);
		HEAPU8[out_buf + copyLen] = 0;
		return copyLen;
	}
	if (globalThis.__krkr_loglevel >= 2) console.log("[JS Bridge] list failed");
	return -1;
});
#endif
...
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
struct tTVPEmscriptenXP3ReadHint
{
	bool active;
	tTVPXP3ReadClass cls;
	const char *mode;
	const char *archive;
	const char *item;
	tjs_uint64 logical_size;
	tjs_uint64 arc_size;
	tjs_uint request_size;
	bool sequential;
};

static tTVPEmscriptenXP3ReadHint TVPEmscriptenXP3ReadHint = {
	false, TVP_XP3_READ_SMALL_RANDOM, "small-random", "", "", 0, 0, 0, false
};

void TVPSetEmscriptenXP3ReadHint(tTVPXP3ReadClass cls, const char *mode,
	const char *archive, const char *item, tjs_uint64 logical_size,
	tjs_uint64 arc_size, tjs_uint request_size, bool sequential)
{
	TVPEmscriptenXP3ReadHint.active = true;
	TVPEmscriptenXP3ReadHint.cls = cls;
	TVPEmscriptenXP3ReadHint.mode = mode ? mode : "unknown";
	TVPEmscriptenXP3ReadHint.archive = archive ? archive : "";
	TVPEmscriptenXP3ReadHint.item = item ? item : "";
	TVPEmscriptenXP3ReadHint.logical_size = logical_size;
	TVPEmscriptenXP3ReadHint.arc_size = arc_size;
	TVPEmscriptenXP3ReadHint.request_size = request_size;
	TVPEmscriptenXP3ReadHint.sequential = sequential;
}

void TVPClearEmscriptenXP3ReadHint()
{
	TVPEmscriptenXP3ReadHint.active = false;
	TVPEmscriptenXP3ReadHint.cls = TVP_XP3_READ_SMALL_RANDOM;
	TVPEmscriptenXP3ReadHint.mode = "small-random";
	TVPEmscriptenXP3ReadHint.archive = "";
	TVPEmscriptenXP3ReadHint.item = "";
	TVPEmscriptenXP3ReadHint.logical_size = 0;
	TVPEmscriptenXP3ReadHint.arc_size = 0;
	TVPEmscriptenXP3ReadHint.request_size = 0;
	TVPEmscriptenXP3ReadHint.sequential = false;
}
#endif

static bool TVPTraceHTTPFontURL(const ttstr &url)
{
	std::string n = url.AsNarrowStdString();
...
#ifdef __EMSCRIPTEN__
static bool TVPHTTPFlagDisabled(const char *name)
{
	tTJSVariant value;
	ttstr argName = ttstr("-") + ttstr(name);
	if(TVPGetCommandLine(argName.c_str(), &value))
	{
		std::string v = ttstr(value.GetString()).AsNarrowStdString();
		return v == "0" || !strcasecmp(v.c_str(), "false") ||
			!strcasecmp(v.c_str(), "off") || !strcasecmp(v.c_str(), "no");
	}
	int urlDisabled = MAIN_THREAD_EM_ASM_INT({
		try {
			var name = UTF8ToString($0);
			var altName = name.split("_").join("-");
			var params = new URLSearchParams(location.search || "");
			for (var i = 0; i < 2; i++) {
				var key = i === 0 ? name : altName;
				if (!params.has(key)) continue;
				var v = String(params.get(key) || "").toLowerCase();
				if (v === "0" || v === "false" || v === "off" || v === "no") return 1;
			}
		} catch (e) {}
		return 0;
	}, name);
	if(urlDisabled) return true;
	return false;
}

static const char * TVPXP3ReadClassName(tTVPXP3ReadClass cls)
{
	switch(cls)
	{
	case TVP_XP3_READ_PSB_FULL: return "psb-full";
	case TVP_XP3_READ_AUDIO_STREAM: return "audio-stream";
	case TVP_XP3_READ_VIDEO_STREAM: return "video-stream";
	case TVP_XP3_READ_SMALL_RANDOM:
	default:
		return "small-random";
	}
}

static tjs_uint TVPGetXP3HintChunkSize(const tTVPEmscriptenXP3ReadHint &hint)
{
	const tjs_uint small = 256 * 1024;
	if(!hint.active || TVPHTTPFlagDisabled("krkr_xp3_adaptive_read")) return small;
	switch(hint.cls)
	{
	case TVP_XP3_READ_PSB_FULL:
		if(TVPHTTPFlagDisabled("krkr_xp3_psb_large")) return small;
		return 2 * 1024 * 1024;
	case TVP_XP3_READ_AUDIO_STREAM:
		if(TVPHTTPFlagDisabled("krkr_xp3_audio_large")) return small;
		return hint.sequential ? 1024 * 1024 : 512 * 1024;
	case TVP_XP3_READ_VIDEO_STREAM:
		if(TVPHTTPFlagDisabled("krkr_xp3_video_large")) return small;
		return hint.sequential ? 2 * 1024 * 1024 : 1024 * 1024;
	case TVP_XP3_READ_SMALL_RANDOM:
	default:
		return small;
	}
...
tTVPHTTPRangeStream::tTVPHTTPRangeStream(const ttstr &url)
{
	URL = url;
	CurrentPos = 0;
	TotalSize = 0;
	...
	TotalSize = emscripten_sync_http_get_size(utf8url.c_str());
	if(TVPTraceHTTPFontURL(URL) && TVPLogL3())
	{
		fprintf(stderr, "[HTTP-FONT] open url=%s size=%llu\n",
			utf8url.c_str(), (unsigned long long)TotalSize);
		fflush(stderr);
	}
#endif
}

tTVPHTTPRangeStream::~tTVPHTTPRangeStream()
{
}

tjs_uint64 TJS_INTF_METHOD tTVPHTTPRangeStream::Seek(tjs_int64 offset, tjs_int whence)
{
	switch(whence)
	{
	case TJS_BS_SEEK_SET: CurrentPos = offset; break;
	case TJS_BS_SEEK_CUR: CurrentPos += offset; break;
	case TJS_BS_SEEK_END: CurrentPos = TotalSize + offset; break;
	}
	return CurrentPos;
}

tjs_uint TJS_INTF_METHOD tTVPHTTPRangeStream::Read(void *buffer, tjs_uint read_size)
{
#ifdef __EMSCRIPTEN__
	std::string utf8url;
	TVPUtf16ToUtf8(utf8url, URL.AsStdString());
	tTVPEmscriptenXP3ReadHint hint = TVPEmscriptenXP3ReadHint;
	tjs_uint bridge_chunk_size = TVPGetXP3HintChunkSize(hint);
	if(bridge_chunk_size < 64 * 1024) bridge_chunk_size = 64 * 1024;
	if(bridge_chunk_size > 4 * 1024 * 1024) bridge_chunk_size = 4 * 1024 * 1024;
	bool trace = TVPTraceHTTPFontURL(URL) && TVPLogL3();
	bool perf = TVPPerfEnabled() && hint.active;
	tjs_uint64 start_pos = CurrentPos;
	tjs_uint request_count = 0;
	tjs_uint max_request_size = 0;
	tjs_uint32 start_tick = perf ? TVPGetRoughTickCount32() : 0;
	if(trace)
	{
		fprintf(stderr, "[HTTP-FONT] read-begin offset=%llu size=%u url=%s\n",
			(unsigned long long)CurrentPos, (unsigned)read_size, utf8url.c_str());
		fflush(stderr);
	}
	tjs_uint total_read = 0;
	while(total_read < read_size)
	{
		tjs_uint request_size = read_size - total_read;
		tjs_uint chunk_remain = bridge_chunk_size - (tjs_uint)(CurrentPos % bridge_chunk_size);
		if(request_size > chunk_remain) request_size = chunk_remain;
		tjs_uint read_bytes = emscripten_sync_http_read(
			utf8url.c_str(), CurrentPos, request_size,
			(tjs_uint8*)buffer + total_read,
			bridge_chunk_size,
			hint.active ? (int)hint.cls : 0,
			hint.active ? hint.item : "");
		request_count++;
		if(request_size > max_request_size) max_request_size = request_size;
		if(read_bytes == 0 && !trace)
		{
			fprintf(stderr, "[HTTP-RANGE] short-read url=%s offset=%llu request=%u total=%u\n",
				utf8url.c_str(),
		...
		if(trace)
		{
			fprintf(stderr, "[HTTP-FONT] read-chunk offset=%llu request=%u got=%u\n",
				(unsigned long long)CurrentPos,
				(unsigned)request_size,
				(unsigned)read_bytes);
			fflush(stderr);
		...
		CurrentPos += read_bytes;
	}
	if(trace)
	{
		fprintf(stderr, "[HTTP-FONT] read-end total=%u final_offset=%llu\n",
			(unsigned)total_read, (unsigned long long)CurrentPos);
		fflush(stderr);
	}
	else if(total_read != read_size)
	{
		fprintf(stderr, "[HTTP-RANGE] read-end-short url=%s requested=%u total=%u final_offset=%llu\n",
			utf8url.c_str(),
			(unsigned)read_size,
			(unsigned)total_read,
			(unsigned long long)CurrentPos);
	...
	if(perf)
	{
		tjs_uint32 end_tick = TVPGetRoughTickCount32();
		fprintf(stderr,
			"[PERF] xp3.read item=%s archive=%s class=%s mode=%s url=%s offset=%llu request=%u actual=%u logical_size=%llu arc_size=%llu seq=%d chunk=%u requests=%u max_request=%u cache=range ms=%u\n",
			hint.item,
			hint.archive,
			TVPXP3ReadClassName(hint.cls),
			hint.mode,
			utf8url.c_str(),
			(unsigned long long)start_pos,
			(unsigned)read_size,
			(unsigned)total_read,
			(unsigned long long)hint.logical_size,
			(unsigned long long)hint.arc_size,
			hint.sequential ? 1 : 0,
			(unsigned)bridge_chunk_size,
			(unsigned)request_count,
			(unsigned)max_request_size,
			(unsigned)(end_tick - start_tick));
		fflush(stderr);
	}
	return total_read;
#else
...
tjs_uint TJS_INTF_METHOD tTVPHTTPRangeStream::Write(const void *buffer, tjs_uint write_size)
{
	return write_size;
}

void TJS_INTF_METHOD tTVPHTTPRangeStream::SetEndOfStorage()
{
}

tjs_uint64 TJS_INTF_METHOD tTVPHTTPRangeStream::GetSize()
{
	return TotalSize;
}

tTJSBinaryStream * TVPCreateHTTPStream(const ttstr& url) {
	return new tTVPHTTPRangeStream(url);
}
//---------------------------------------------------------------------------
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/deleteFile)
{
	...
	if(local.IsEmpty())
	{
		TVPThrowExceptionMessage(TVPCannotGetLocalName, storage);
	}

	bool res = TVPRemoveFile(local);
	if(res) TVPClearStorageCaches();
