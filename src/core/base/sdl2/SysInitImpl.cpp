...
void TVPBeforeSystemInit()
{
	...
	if (!TVPProjectDirSelected)
	{
		...
		KRKR_LOG_L2("TVPProjectDirSelected before args: %d, argc=%d\n", TVPProjectDirSelected, _argc);
		...
		{
			if (TVPLogL2()) {
				tjs_int len = TVPWideCharToUtf8String(_wargv[i], NULL);
				std::vector<char> narrow((size_t)len + 1);
				TVPWideCharToUtf8String(_wargv[i], narrow.data());
				KRKR_LOG_L2("argv[%d]: %s\n", i, narrow.data());
			}
			if (_wargv[i][0] == TJS_W('-') && _wargv[i][1] == TJS_W('-') && _wargv[i][2] == 0)
			{
				break;
			...
			if (_wargv[i][0] != TJS_W('-'))
			{
				...
				if (!TJS_strncmp(dirbuf.c_str(), TJS_W("http://"), 7) || !TJS_strncmp(dirbuf.c_str(), TJS_W("https://"), 8))
				{
					if (dirbuf.GetLastChar() != TJS_W('>') && dirbuf.GetLastChar() != TJS_W('/')) {
						dirbuf += TJS_W(">");
					}
					TVPProjectDirSelected = true;
				}
				else if (TVPCheckExistentLocalFolder(dirbuf))
				{
					dirbuf += TJS_W("/");
					TVPProjectDirSelected = true;
				}
	...
	{
		TVPAddImportantLog( TVPFormatMessage(TVPInfoSelectedProjectDirectory, TVPProjectDir) );
		extern void TVPAddAutoPath(const ttstr & name);
		TVPAddAutoPath(TVPProjectDir);

		ttstr basePath = TVPProjectDir;
		if (basePath.GetLastChar() == TJS_W('>')) {
			const tjs_char* sharp = TJS_strchr(basePath.c_str(), TJS_W('>'));
			if (sharp != NULL) {
				basePath = ttstr(basePath.c_str(), (int)(sharp - basePath.c_str()));
				basePath = TVPNormalizeStorageName(TVPExtractStoragePath(basePath));
			}
		}

		extern bool TVPIsExistentStorage(const ttstr &name);
		const tjs_char* archives[] = {
			TJS_W("data.xp3"), TJS_W("patch.xp3"), TJS_W("main.xp3"), TJS_W("dtcn.xp3"),
			TJS_W("scenario.xp3"), TJS_W("scn.xp3"), TJS_W("sound.xp3"), TJS_W("bgm.xp3"),
			TJS_W("bgimage.xp3"), TJS_W("evimage.xp3"), TJS_W("fgimage.xp3"), TJS_W("image.xp3"),
			TJS_W("video.xp3"), TJS_W("voice.xp3"), NULL
		};
		for (int i = 0; archives[i] != NULL; ++i) {
			ttstr archivePath = basePath + archives[i];
			if (TVPIsExistentStorage(archivePath)) {
				TVPAddAutoPath(archivePath + TJS_W(">"));
			}
		}
	}
}
...
void TVPAfterSystemInit()
{
...
#if defined(__EMSCRIPTEN__)
	{
		...
		if (global) {
			...
			// Register KAGParserEx (full replacement, exposes TJS class name "KAGParser")
			// Must init static helpers (Dictionary.assign / Array.assign caches) first.
			TVPKAGParserEX_StaticInit();
			tjsclass = TVPCreateNativeClass_KAGParserEX();
		...
		// Execute stub TJS script after all classes are registered
		extern void TVPExecuteExpression(const ttstr &expr, tTJSVariant *result);
		ttstr stubScript = TJS_W(
			"class StubObject {"
			"  var _name, caption, checked, enabled, visible, children, group, radio, shortcut, onClick, index, parent;"
			"  function StubObject(n) {"
			"    _name = n;"
			"    caption = '';"
			"    checked = false;"
			"    enabled = true;"
			...
			"    parent = null;"
			"  }"
			"  function toString() { return '[Stub:' + _name + ']'; }"
			"  function popupEx(x,y) {}"
			"  function add(item) { children.add(item); }"
			"  function remove(item) { children.remove(item); }"
			"  function __saveSystemVariablesAfterMenuClick() {"
			"    try {"
			"      if (typeof Scripts != 'undefined' && typeof Scripts.requestSystemVariablesSave == 'Object')"
			"        Scripts.requestSystemVariablesSave('StubMenuItem.click:' + _name);"
			"    } catch(e) {"
			"      try { Debug.message('[SYSVAR-SAVE] source=StubMenuItem.click request-failed name=' + _name + ' message=' + e.message); } catch(ignore) {}"
			"    }"
			"  }"
			"  function click() { if (typeof onClick == 'Object') onClick(); __saveSystemVariablesAfterMenuClick(); }"
			"  function fireClick() { click(); }"
			"  function count() { return children.count; }"
			"  function insert(item, index) { children.insert(index, item); }"
			"  function clear() { children.clear(); }"
			"  function getItem(index) { return children[index]; }"
			"  function assign(source) {}"
			"  function save(filename) { return ''; }"
			"  function load(filename) {}"
			"};"
			"var stubClasses = ['MenuItem', 'Menu', 'SystemMenu', 'TrayIcon', 'FileDialog', 'ColorDialog', 'FontDialog', 'PrintDialog', 'IniFile', 'Registry', 'WSEvent', 'WinAPI', 'ShellLink', 'WIN32Dialog', 'WIN32DialogEX', 'WIN32GenericDialog', 'WIN32GenericDialogEX'];"
			"for (var i = 0; i < stubClasses.length; i++) {"
			"  var name = stubClasses[i];"
			"  if (typeof global[name] === 'undefined') {"
			"    try {"
			"      Scripts.eval('class ' + name + ' extends StubObject { function ' + name + '() { super.StubObject(\"' + name + '\"); } }');"
			"      Debug.message('[Stub System] Stubbed ' + name);"
			"    } catch(e) {"
			"      Debug.message('[Stub System] Failed to stub ' + name + ': ' + e.message);"
			"    }"
			"  } else {"
			"    Debug.message('[Stub System] Already defined: ' + name);"
			"  }"
			"}"
			"if(typeof global.__WindowMenu === 'undefined') {"
			"  global.__WindowMenu = new StubObject('Window.menu');"
			"}"
			"try { if (typeof Debug.console === 'undefined') Debug.console = new StubObject('Debug.console'); } catch(e) {}"
			"if (typeof System.inform !== 'undefined') {"
			"  System.inform = function(msg) { Debug.message('[Inform blocked]: ' + msg); };"
			"}"
			"try {"
			"  Scripts.eval('class MessageFrameChanger extends StubObject { function MessageFrameChanger() { super.StubObject(\"MessageFrameChanger\"); } function getWindowColor() { return 0; } function getFrameColor() { return 0; } }');"
			"  if(typeof global.MessageFrameChanger === 'undefined') global.MessageFrameChanger = MessageFrameChanger;"
			"} catch(e) {}"
			"try {"
			"  if(typeof Window !== 'undefined' && typeof Window.messageFrameChanger === 'undefined') {"
			"    Window.messageFrameChanger = new MessageFrameChanger();"
			"  }"
				"} catch(e) {}"
				"global.__emscripten_stub_system_loaded=true;"
					"Debug.message('[Emscripten Stub System] Win32 UI stubs initialized');"
					"if (false && typeof global.Motion === 'undefined') {"
			"  global.Motion = %[];"
			...
			"    function load(filename) { var id = _nextId++; return id; }"
			"    function loadPSB(filename) { var id = _nextId++; return id; }"
			"    function loadPSBFromMemory(buf) { var id = _nextId++; return id; }"
			...
			"    function load(filename) { return 0; }"
			"    function loadPSB(filename) { return 0; }"
			...
			"  global.__WindowMotionManager = new Motion.ResourceManager();"
			"  Debug.message('[Emscripten Stub System] Motion classes initialized');"
			"}"
			"try {"
			"  if (typeof Window !== 'undefined') {"
			"    global.dtNone = 0; global.dtDrawDib = 1; global.dtDBGDI = 2; global.dtDBDD = 3; global.dtDBD3D = 4;"
			"    Debug.message('[Emscripten Stub System] dtDrawDib constants injected');"
			"  }"
			"} catch(e) { Debug.message('[Stub] dtDrawDib injection failed: ' + e.message); }"
		);
		try {
			TVPExecuteExpression(stubScript, nullptr);
		} catch(...) {
			TVPAddLog(TJS_W("Warning: Emscripten stub script failed to execute"));
		}
	}
#endif
...
static std::vector<std::string> * TVPGetConfigFileOptions(const tjs_string& filename)
{
	...
	else if(!TVPCheckExistentLocalFile(filename.c_str()))
		errmsg = TJS_W("not a local file");
	...
	if(errmsg == TJS_W(""))
	{
		try
		{
			...
			if(ret == NULL)
				errmsg = TJS_W("LoadLinesFromFile returned null");
		}
		catch(Exception & e)
		{
			errmsg = e.what();
	...
	if(TVPLogL2()) {
		...
		TVPAddLog(ttstr(TJS_W("[config-read] probe path=")) + filename +
			TJS_W(" exists=") + ttstr(FileExists(filename) ? TJS_W("1") : TJS_W("0")) +
			TJS_W(" local=") + ttstr(TVPCheckExistentLocalFile(filename.c_str()) ? TJS_W("1") : TJS_W("0")) +
			TJS_W(" lines=") + ttstr(ret ? (tjs_int)ret->size() : (tjs_int)-1));
	}
