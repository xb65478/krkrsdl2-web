...
// Shared-memory event queue for cross-thread delivery in Emscripten pthreads.
// SDL_PushEvent/SDL_PollEvent uses thread-local storage in Emscripten,
// so events pushed from the timer pthread are invisible to the main loop pthread.
static std::mutex s_nativeEventMutex;
static std::vector<NativeEvent*> s_nativeEventPending;
...
void NativeEventQueueImplement::PostEvent(const NativeEvent& ev) {
	...
	std::lock_guard<std::mutex> lock(s_nativeEventMutex);
	s_nativeEventPending.push_back(tmp_ev);
}

void NativeEventQueueImplement::DrainNativeEventQueue() {
	std::vector<NativeEvent*> pending;
	{
		std::lock_guard<std::mutex> lock(s_nativeEventMutex);
		pending.swap(s_nativeEventPending);
	}
	for (auto* ev : pending) {
		ev->HandleEvent();
	}
...
#else
// Non-pthreads path: use SDL custom events
tjs_uint32 NativeEventQueueImplement::native_event_queue_custom_event_type = 0;
