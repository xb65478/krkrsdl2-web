...
class tTJSNI_VideoOverlay : public tTJSNI_BaseVideoOverlay
{
	...
	#if KRKRSDL2_FFMPEG_ENABLED
		krkr::video::FFmpegVideoPlayer *FFmpegPlayer;
		double LastTickMs;
		class tTVPBaseBitmap	*FFmpegBitmap[2];
		int FFmpegBitmapIndex;
		void EnsureFFmpegLayerFrameBuffer(int width, int height);
		void PresentFFmpegFrameToLayers();
		void PresentFFmpegFrameToSurface();
	#endif
...
#if KRKRSDL2_FFMPEG_ENABLED
	void TickFFmpegVideo(); // Called from main loop to advance video decode
#endif
