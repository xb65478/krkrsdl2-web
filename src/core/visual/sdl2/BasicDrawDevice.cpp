...
void TJS_INTF_METHOD tTVPBasicDrawDevice::StartBitmapCompletion(iTVPLayerManager * manager)
{
	if (Window)
	{
		...
		if (form)
		{
			...
			KRKR_LOG_L3("[SBC] StartBitmapCompletion: bc=%p bc_surface=%p\n",
				(void*)bitmap_completion,
				bitmap_completion ? (void*)bitmap_completion->surface : nullptr);
		}
		else
		{
			KRKR_LOG_L3("[SBC] StartBitmapCompletion: form is null\n");
		}
	}
	else
	{
		KRKR_LOG_L3("[SBC] StartBitmapCompletion: Window is null\n");
	}
#if 0
	EnsureDevice();
...
//----------------------------------------------------------------------
#define TVP_REGISTER_BDD_ENUM(name) \
...
TVP_REGISTER_BDD_ENUM(dtNone)
TVP_REGISTER_BDD_ENUM(dtDrawDib)
TVP_REGISTER_BDD_ENUM(dtDBGDI)
TVP_REGISTER_BDD_ENUM(dtDBDD)
TVP_REGISTER_BDD_ENUM(dtDBD3D)
#undef TVP_REGISTER_BDD_ENUM
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(preferredDrawer)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		*result = (tjs_int64)tTVPBasicDrawDevice::dtDrawDib;
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
