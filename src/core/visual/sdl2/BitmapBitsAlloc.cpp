...
void tTVPBitmapBitsAlloc::CheckSentinels(void* ptr, const char* context) {
	if(!ptr) return;
	...
	bool underrun = (~(*(tjs_uint32*)(bptr - sizeof(tjs_uint32))) != record->sentinel_backup1);
	bool overrun = (~(*(tjs_uint32*)(bptr + record->size)) != record->sentinel_backup2);

	if(underrun || overrun) {
		fprintf(stderr, "[SENTINEL] %s: ptr=%p size=%u %s%s\n",
			context, ptr, record->size,
			underrun ? "UNDERRUN " : "",
			overrun ? "OVERRUN" : "");
		fflush(stderr);
	}
