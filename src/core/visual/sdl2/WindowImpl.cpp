...
#define DECLARE_DUMMY_PROP(NAME) \
...
TJS_END_NATIVE_PROP_DECL_OUTER(cls, NAME)

DECLARE_DUMMY_PROP(innerSunken)
DECLARE_DUMMY_PROP(outerSunken)
DECLARE_DUMMY_PROP(showScrollBars)
...
TJS_BEGIN_NATIVE_PROP_DECL(menu)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		...
		if (global) {
			global->PropGet(0, TJS_W("__WindowMenu"), NULL, result, global);
			global->Release();
		}
		return TJS_S_OK;
	}
...
TJS_END_NATIVE_PROP_DECL_OUTER(cls, menu)
#endif
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
TJS_BEGIN_NATIVE_PROP_DECL(motion_manager)
{
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		extern iTJSDispatch2 * TVPGetScriptDispatch();
		iTJSDispatch2 * global = TVPGetScriptDispatch();
		if (global) {
			global->PropGet(0, TJS_W("__WindowMotionManager"), NULL, result, global);
			global->Release();
		}
		return TJS_S_OK;
	}
	...
	TJS_BEGIN_NATIVE_PROP_SETTER
	{
		...
		iTJSDispatch2 * global = TVPGetScriptDispatch();
		if (global) {
			global->PropSet(TJS_MEMBERENSURE, TJS_W("__WindowMotionManager"), NULL, param, global);
			global->Release();
		}
		return TJS_S_OK;
	}
...
TJS_END_NATIVE_PROP_DECL_OUTER(cls, motion_manager)
#endif
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
// mainwindow.tjs calls skipToSync() via conductor.tjs trigger on every
// click/key event. Without it the conductor cannot advance the scenario.
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/skipToSync)
{
	return TJS_S_OK;
}
TJS_END_NATIVE_METHOD_DECL_OUTER(cls, /*func. name*/skipToSync)
#endif
//---------------------------------------------------------------------------
// windowEx compatibility: draw device type constants as native properties
#define TVP_REGISTER_DT_CONST(name, value) \
TJS_BEGIN_NATIVE_PROP_DECL(name) \
{ \
	TJS_BEGIN_NATIVE_PROP_GETTER \
	{ \
		*result = (tjs_int64)(value); \
		return TJS_S_OK; \
	} \
	TJS_END_NATIVE_PROP_GETTER \
	TJS_DENY_NATIVE_PROP_SETTER \
...
TVP_REGISTER_DT_CONST(dtNone, 0)
TVP_REGISTER_DT_CONST(dtDrawDib, 1)
TVP_REGISTER_DT_CONST(dtDBGDI, 2)
TVP_REGISTER_DT_CONST(dtDBDD, 3)
TVP_REGISTER_DT_CONST(dtDBD3D, 4)
#undef TVP_REGISTER_DT_CONST
//---------------------------------------------------------------------------
#ifdef __EMSCRIPTEN__
	{
		tTJSVariant val;
		tjs_error hr;
		KRKR_LOG_L2("[DIAG-C] About to check skipToSync on cls=%p\n", (void*)cls);
		hr = cls->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("skipToSync"), NULL, &val, cls);
		KRKR_LOG_L2("[DIAG-C] skipToSync on cls: hr=%d\n", (int)hr);
		hr = cls->PropGet(TJS_MEMBERMUSTEXIST, TJS_W("dtDrawDib"), NULL, &val, cls);
		KRKR_LOG_L2("[DIAG-C] dtDrawDib on cls: hr=%d val=%d\n", (int)hr, (int)(tjs_int)val);
	}
#endif
//---------------------------------------------------------------------------
