	 ...
	 , image_load_thread_(NULL), has_map_report_process_(false), console_cache_(1024)
{
	...
	savedata_sync_request_count_ = 0;
	savedata_sync_start_count_ = 0;
}
...
bool tTVPApplication::StartApplication( int argc, tjs_char* argv[] ) {
	...
	try {
		...
		KRKR_LOG_L2("BEFORE TVPInitScriptEngine\n");
		TVPInitScriptEngine();
		engine_init = true;
		KRKR_LOG_L2("AFTER TVPInitScriptEngine\n");

		// banner
		KRKR_LOG_L2("BEFORE TVPGetOSName\n");
		ttstr osname = TVPGetOSName();
		KRKR_LOG_L2("BEFORE TVPGetPlatformName\n");
		ttstr platname = TVPGetPlatformName();
		KRKR_LOG_L2("BEFORE TVPFormatMessage\n");
		ttstr msg = TVPFormatMessage(TVPProgramStartedOn, osname, platname);
		KRKR_LOG_L2("BEFORE TVPAddImportantLog\n");
		TVPAddImportantLog( msg );
		KRKR_LOG_L2("AFTER TVPAddImportantLog\n");
...
void tTVPApplication::PrintConsole( const tjs_char* mes, unsigned long len, bool iserror ) {
	KRKR_LOG_L3("PrintConsole: len=%lu\n", len);
	if (!mes) { KRKR_LOG_L3("PrintConsole: mes is null\n"); return; }
	if( console_cache_.size() < (len*3+1) ) {
		console_cache_.resize(len*3+1);
	}
	KRKR_LOG_L3("PrintConsole: before TVPWideCharToUtf8String\n");
	tjs_int u8len = TVPWideCharToUtf8String( mes, &(console_cache_[0]) );
	KRKR_LOG_L3("PrintConsole: after TVPWideCharToUtf8String u8len=%d\n", u8len);
	...
	KRKR_LOG_L3("PrintConsole: after fprintf\n");
#endif
#if 0
#ifdef _DEBUG
...
#endif
#ifdef __EMSCRIPTEN__
	static int run_call_count = 0;
	run_call_count++;
	double run_start_ms = emscripten_get_now();
	double after_sdl_ms = run_start_ms;
	double after_idle_ms = run_start_ms;
	double after_tick_ms = run_start_ms;
	if (run_call_count <= 5 || run_call_count % 500 == 0) {
		KRKR_LOG_L3("[DIAG-RUN] Run() call #%d\n", run_call_count);
	}
#endif
	sdl_process_events();
#ifdef __EMSCRIPTEN__
	after_sdl_ms = emscripten_get_now();
#endif
	if (tarminate_)
	{
		return;
...
#ifdef __EMSCRIPTEN__
	after_idle_ms = emscripten_get_now();
#endif
#ifdef __EMSCRIPTEN__
	if (run_call_count <= 5 || run_call_count % 500 == 0) {
		KRKR_LOG_L3("[DIAG-RUN] Run() #%d after ApplicationIdle, done=%d\n", run_call_count, (int)done);
	}
#endif
...
#ifdef __EMSCRIPTEN__
	after_tick_ms = emscripten_get_now();
	TVPLogMotionHostBurstTrace("run",
		after_sdl_ms - run_start_ms,
		after_idle_ms - after_sdl_ms,
		after_tick_ms - after_idle_ms,
		done ? 1.0 : 0.0);
#endif
	TVPFlushPendingSystemVariablesSave("run-loop", false);
#ifdef __EMSCRIPTEN__
	if (!syncfs_is_finished_)
	{
		tjs_uint finished_count = (tjs_uint)MAIN_THREAD_EM_ASM_INT({
			return (globalThis.__krkrEngineSavedataSyncFinishedCount || 0) | 0;
		});
		if (finished_count >= savedata_sync_start_count_)
		{
			FinishedSyncSavedata();
		}
	}
#endif
...
#ifdef __EMSCRIPTEN__
		savedata_sync_start_count_++;
		KRKR_LOG_L2("[IDBFS-SYNC] start count=%u\n", (unsigned)savedata_sync_start_count_);
		MAIN_THREAD_EM_ASM({
			var count = $0;
			function markFinished(err) {
				if (err) console.error("[IDBFS-SYNC] js-finish error=" + err + " count=" + count);
				else if ((globalThis.__krkr_loglevel || 1) >= 2) console.log("[IDBFS-SYNC] js-finish ok count=" + count);
				globalThis.__krkrEngineSavedataSyncFinishedCount =
					Math.max(globalThis.__krkrEngineSavedataSyncFinishedCount || 0, count);
			}
			try {
				if (typeof globalThis.__krkrRequestSavedataSync === "function") {
					globalThis.__krkrRequestSavedataSync({
						source: "engine",
						count: count,
						delayMs: 120,
						finish: markFinished
					...
					if ((globalThis.__krkr_loglevel || 1) >= 2) console.warn("[IDBFS-SYNC] main-thread-fallback start count=" + count);
					FS.syncfs(false, markFinished);
				}
			} catch (e) {
				markFinished(e);
			}
...
std::vector<std::string>* LoadLinesFromFile( const tjs_string& path ) {
	...
	if( !TVPUtf16ToUtf8(path_utf8, path) ) {
		TVPAddLog(ttstr(TJS_W("[config-read] utf16->utf8 failed path=")) + path);
		return NULL;
	}
	FILE *fp = fopen(path_utf8.c_str(), "rb");
	if( fp == NULL ) {
		TVPAddLog(ttstr(TJS_W("[config-read] fopen failed path=")) + path +
			TJS_W(" errno=") + ttstr((tjs_int)errno));
		return NULL;
	}
	char buff[1024];
	...
	fclose(fp);
	TVPAddLog(ttstr(TJS_W("[config-read] loaded path=")) + path +
		TJS_W(" lines=") + ttstr((tjs_int)ret->size()));
	return ret;
}
