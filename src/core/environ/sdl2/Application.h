...
class tTVPApplication {
	...
	tjs_uint savedata_sync_request_count_;
	tjs_uint savedata_sync_start_count_;
...
public:
	...
	static inline int MessageDlg( const tjs_string& string, const tjs_string& caption, int type, int button ) {
		...
		if (TVPUtf16ToUtf8(s_utf8, s_utf16) && TVPUtf16ToUtf8(c_utf8, c_utf16))
		{
			...
			emscripten_console_logf("[MessageDlg] %s: %s", c_utf8.c_str(), s_utf8.c_str());
#else
	...
	{
#ifdef __EMSCRIPTEN__
		savedata_sync_request_count_++;
		KRKR_LOG_L2("[IDBFS-SYNC] request count=%u inProgress=%d\n",
			(unsigned)savedata_sync_request_count_, syncfs_is_finished_ ? 0 : 1);
#endif
	...
	{
#ifdef __EMSCRIPTEN__
		KRKR_LOG_L2("[IDBFS-SYNC] finish count=%u\n", (unsigned)savedata_sync_start_count_);
#endif
