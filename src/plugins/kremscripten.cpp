...
class iTJSDispatch2WrapperForEmscripten : public tTJSDispatch
{
	...
	public: tjs_error TJS_INTF_METHOD CreateNew(tjs_uint32 flag, const tjs_char * membername, tjs_uint32 *hint, iTJSDispatch2 **result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
	{
		...
		if (result) {
			tTJSVariant var = emscripten_val_to_tjs_variant(js_reflect_construct(v, temp_array));
			*result = var.AsObject();
	...
	public: tjs_error TJS_INTF_METHOD FuncCall(tjs_uint32 flag, const tjs_char * membername, tjs_uint32 *hint, tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
	{
		...
		{
			if (param[i] == nullptr) {
				emscripten_log(EM_LOG_CONSOLE, "param[%d] is NULL!", i);
				continue;
			}
			tTJSVariantType vt = param[i]->Type();
			emscripten_log(EM_LOG_CONSOLE, "param[%d] type: %d", i, (int)vt);
			if (vt == tvtObject) {
				emscripten_log(EM_LOG_CONSOLE, "param[%d] object: %p %p", i, param[i]->AsObjectNoAddRef(), param[i]->AsObjectThisNoAddRef());
			}
			temp_array.call<void>("push", tjs_variant_to_emscripten_val(*param[i]));
		}
		...
		if (result) *result = emscripten_val_to_tjs_variant(js_reflect_apply(func_to_call, vthis, temp_array));
		return TJS_S_OK;
	}
	...
	public: tjs_error TJS_INTF_METHOD PropGet(tjs_uint32 flag, const tjs_char * membername, tjs_uint32 *hint, tTJSVariant *result, iTJSDispatch2 *objthis)
	{
		...
		if (result) *result = emscripten_val_to_tjs_variant(v[tjs_string(membername)], vthis);
		return TJS_S_OK;
	}
	...
	public: tjs_error TJS_INTF_METHOD PropGetByNum(tjs_uint32 flag, tjs_int num, tTJSVariant *result, iTJSDispatch2 *objthis)
	{
		...
		if (result) *result = emscripten_val_to_tjs_variant(v[(tjs_int32)num], vthis);
		return TJS_S_OK;
	}
...
emscripten::val getAudioDiagState()
{
	emscripten::val result = emscripten::val::object();
#if KRKRSDL2_FFMPEG_ENABLED
	result.set("ffmpegEnabled", true);
	result.set("ffmpegWaveOpenCount", static_cast<double>(KRKRSDL2AudioDiagGetFFmpegWaveOpenCount()));
	result.set("ffmpegWaveRenderCallCount", static_cast<double>(KRKRSDL2AudioDiagGetFFmpegWaveRenderCallCount()));
	result.set("ffmpegWaveRenderedSamples", static_cast<double>(KRKRSDL2AudioDiagGetFFmpegWaveRenderedSamples()));
	result.set("ffmpegWaveRenderedBytes", static_cast<double>(KRKRSDL2AudioDiagGetFFmpegWaveRenderedBytes()));
#else
	result.set("ffmpegEnabled", false);
	result.set("ffmpegWaveOpenCount", 0.0);
	result.set("ffmpegWaveRenderCallCount", 0.0);
	result.set("ffmpegWaveRenderedSamples", 0.0);
	result.set("ffmpegWaveRenderedBytes", 0.0);
#endif
	result.set("openALCreateStreamCount", static_cast<double>(KRKRSDL2AudioDiagGetOpenALCreateStreamCount()));
	result.set("openALAppendBufferCount", static_cast<double>(KRKRSDL2AudioDiagGetOpenALAppendBufferCount()));
	result.set("openALAppendBytes", static_cast<double>(KRKRSDL2AudioDiagGetOpenALAppendBytes()));
	result.set("openALPlayCount", static_cast<double>(KRKRSDL2AudioDiagGetOpenALPlayCount()));
	result.set("openALAppendBufferMs", static_cast<double>(KRKRSDL2AudioDiagGetOpenALAppendBufferMsX1000()) / 1000.0);
	result.set("openALLowQueuedCount", static_cast<double>(KRKRSDL2AudioDiagGetOpenALLowQueuedCount()));
	result.set("openALMinQueuedBuffers", static_cast<double>(KRKRSDL2AudioDiagGetOpenALMinQueuedBuffers()));
	result.set("wasmPumpTriggerCount", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpTriggerCount()));
	result.set("wasmPumpFillBufferCount", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpFillBufferCount()));
	result.set("wasmPumpTriggerMs", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpTriggerMsX1000()) / 1000.0);
	result.set("wasmPumpFillBufferMs", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpFillBufferMsX1000()) / 1000.0);
	result.set("wasmPumpDecodeCount", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpDecodeCount()));
	result.set("wasmPumpDecodeMs", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpDecodeMsX1000()) / 1000.0);
	result.set("wasmPumpReadL2Count", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpReadL2Count()));
	result.set("wasmPumpReadL2Ms", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpReadL2MsX1000()) / 1000.0);
	result.set("wasmPumpAppendMs", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpAppendMsX1000()) / 1000.0);
	result.set("wasmPumpLowL2RemainCount", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpLowL2RemainCount()));
	result.set("wasmPumpMinL2Remain", static_cast<double>(KRKRSDL2AudioDiagGetWasmPumpMinL2Remain()));
	return result;
}

emscripten::val probeFFmpegAVIO(tjs_string storageName, tjs_string expectedKind)
{
	emscripten::val result = emscripten::val::object();
	std::string storage = ttstr(storageName).AsNarrowStdString();
	std::string expected = ttstr(expectedKind).AsNarrowStdString();
	result.set("storage", storage);
	result.set("expectedKind", expected);
#if KRKRSDL2_FFMPEG_ENABLED
	auto runProbe = [&]() {
		krkr::avio::FFmpegAVIOBridge bridge;
		AVFormatContext *formatContext = nullptr;
		bool opened = bridge.openFromKRKRStorage(storage, &formatContext);
		result.set("ok", opened);
		result.set("ffmpegEnabled", true);
		result.set("streamSize", static_cast<double>(bridge.getStreamSize()));
		result.set("readCalls", bridge.getReadCallCount());
		result.set("seekCalls", bridge.getSeekCallCount());
		result.set("readErrors", bridge.getReadErrorCount());
		result.set("seekErrors", bridge.getSeekErrorCount());
		result.set("eofCount", bridge.getEofCount());
		result.set("bytesRead", static_cast<double>(bridge.getBytesRead()));
		if (!opened || !formatContext) {
			result.set("error", "open-or-probe-failed");
		} else {
			result.set("format", formatContext->iformat ? formatContext->iformat->name : "");
			result.set("formatLongName", formatContext->iformat ? formatContext->iformat->long_name : "");
			if (formatContext->duration != AV_NOPTS_VALUE) {
				result.set("durationSec", formatContext->duration / static_cast<double>(AV_TIME_BASE));
			} else {
				result.set("durationSec", -1.0);
			}
			result.set("bitRate", static_cast<double>(formatContext->bit_rate));
			result.set("streamCount", static_cast<int>(formatContext->nb_streams));
			...
			for (unsigned int i = 0; i < formatContext->nb_streams; i++) {
				...
				item.set("type", typeName ? typeName : "");
				const AVCodec *codec = avcodec_find_decoder(par->codec_id);
				item.set("codec", codec ? codec->name : "");
				item.set("codecId", static_cast<int>(par->codec_id));
				item.set("duration", stream->duration == AV_NOPTS_VALUE ? -1.0 :
					stream->duration * av_q2d(stream->time_base));
				item.set("timeBaseNum", stream->time_base.num);
				item.set("timeBaseDen", stream->time_base.den);
				if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
					audioCount++;
					item.set("sampleRate", par->sample_rate);
					item.set("channels", par->ch_layout.nb_channels);
					item.set("bitRate", static_cast<double>(par->bit_rate));
				} else if (par->codec_type == AVMEDIA_TYPE_VIDEO) {
					videoCount++;
					item.set("width", par->width);
					item.set("height", par->height);
					item.set("fps", stream->avg_frame_rate.den != 0 ? av_q2d(stream->avg_frame_rate) : 0.0);
					const char *pixFmtName = av_get_pix_fmt_name(static_cast<AVPixelFormat>(par->format));
					item.set("pixFmt", pixFmtName ? pixFmtName : "");
				}
				streams.call<void>("push", item);
			}
			result.set("audioStreamCount", audioCount);
			result.set("videoStreamCount", videoCount);
			result.set("streams", streams);
			bool kindOk = true;
			if (expected == "audio") kindOk = audioCount > 0;
			else if (expected == "video") kindOk = videoCount > 0;
			result.set("kindOk", kindOk);
			result.set("ok", opened && kindOk && bridge.getReadErrorCount() == 0 && bridge.getSeekErrorCount() == 0);
		}
	};
	if (TVPGetScriptEngine()) {
		...
		try {
			...
			result.set("ok", false);
			result.set("ffmpegEnabled", true);
			result.set("error", "unknown-native-exception");
		}
	}
#else
	result.set("ok", false);
	result.set("ffmpegEnabled", false);
	result.set("error", "KRKRSDL2_FFMPEG_ENABLED=0");
#endif
	return result;
}
...
EMSCRIPTEN_BINDINGS(KirikiriEmscriptenInterface)
{
	emscripten::function("evalTJS", &evalTJSFromJS);
		...
		emscripten::function("getAudioDiagState", &getAudioDiagState);
		emscripten::function("probeFFmpegAVIO", &probeFFmpegAVIO);
